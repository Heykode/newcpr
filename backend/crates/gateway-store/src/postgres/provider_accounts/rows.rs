...
pub struct RotateProviderAccount {
    ...
    pub relogin_operation_id: Option<String>,
    pub audit: AdminAuditEvent,
}
...
impl fmt::Debug for RotateProviderAccount {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            ...
            .field("relogin_operation_id", &self.relogin_operation_id)
            .field("audit", &self.audit)
            .finish()
    }
}
...
pub(crate) const ACCOUNT_SELECT: &str = "select
            ...
            has_refresh_token, access_token_expires_at, next_refresh_at, enabled, turn_state_injection_enabled, responses_upstream, excel_models, excel_models_follow_global, excel_cache_creation_as_input, excel_auto_disable_on_403,
...
pub(crate) fn core_account_from_summary(
    ...
    .with_excel_auto_disable_on_403(summary.excel_auto_disable_on_403)
