...
pub(crate) async fn load_admin_account_page(
    ...
    let plan_type = query.plan_type.as_deref();
    ...
    let statement = format!(
        "with account_statuses as (
                   ...
                   ($7 = 1 and exists (
                     select 1
                      ...
                      where membership.provider_account_id = a.id
                        and membership.account_group_id = $8
                   )) or
                   ($7 = 2 and not exists (
                     select 1
                       from account_group_accounts membership
         ...
         select (select case when auto_location then detected_location_json -> 'location' else request_location_json end from outbound_proxies where outbound_proxies.id = a.outbound_proxy_id) as request_location_json,
                ...
                a.next_refresh_at, a.enabled, a.turn_state_injection_enabled, a.responses_upstream, a.excel_models, a.excel_models_follow_global, excel_cache_creation_as_input, excel_auto_disable_on_403,
...
fn admin_account_order(sort: Option<AdminAccountSort>) -> String {
    let Some(sort) = sort else {
        return "a.created_at desc, a.id desc".to_owned();
    };
    let expression = match sort.field {
        ...
        AdminAccountSortField::ReloginCount => "a.relogin_count",
        AdminAccountSortField::CreatedAt => "a.created_at",
