...
async fn load_member_facts<'e>(
    ...
    let rows = sqlx::query(
        "select membership.account_group_id,
                ...
                account.next_refresh_at, account.enabled, account.turn_state_injection_enabled, account.responses_upstream, account.excel_models, account.excel_models_follow_global,
