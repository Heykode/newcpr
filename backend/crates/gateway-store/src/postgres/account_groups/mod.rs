...
impl AccountGroupStore for PgAccountGroupRepository {
    ...
    async fn load_group_monitor(
        &self,
        observed_at: chrono::DateTime<chrono::Utc>,
    ) -> AdminStoreResult<gateway_admin::model::group_monitor::GroupMonitorFacts> {
        self.monitor_facts(observed_at).await
    }
    ...
    async fn save_group_monitor(
        ...
        self.save_monitor_snapshot(report, config_revision)
            .await
            .map_err(|error| admin_store_error(ENTITY, error))
    }
    ...
    async fn read_group_monitor(
        ...
        self.read_monitor_snapshot(group_ids)
            .await
            .map_err(|error| admin_store_error(ENTITY, error))
    }
    ...
    async fn load_account_group_members(
        ...
        load_member_facts(&self.pool, group_ids)
            .await
            .map_err(|error| admin_store_error(ENTITY, error))
    }
...
async fn load_member_facts<'e>(
    executor: impl sqlx::PgExecutor<'e>,
    group_ids: &[AccountGroupId],
) -> StoreResult<Vec<AccountGroupMemberFact>> {
    ...
    let rows = sqlx::query(
        "select membership.account_group_id,
                ...
                account.next_refresh_at, account.enabled, account.turn_state_injection_enabled, account.responses_upstream, account.excel_models, account.excel_models_follow_global, excel_cache_creation_as_input, excel_auto_disable_on_403,
                case when account.excel_models_follow_global then (select excel_default_models from runtime_settings where id = 1) else account.excel_models end as effective_excel_models,
                ...
                account.created_at, account.updated_at, account.relogin_count, account.last_relogin_at,
    ...
    ).bind(group_ids).fetch_all(executor).await
        .map_err(|_| unavailable("load account group members"))?;
    rows.into_iter()
        .map(|row| {
