...
fn visit(value: &mut Value, key: &str, media: bool, depth: usize, nodes: &mut usize) {
    ...
    *nodes -= 1;
    ...
    match value {
        Value::Object(fields) => {
            let media_container = matches!(
                normalized.as_str(),
                "image" | "imageurl" | "audio" | "inputaudio" | "video" | "file"
            ...
            for (name, value) in fields {
                let payload = name.to_ascii_lowercase().replace(['-', '_'], "");
                if !media
                    && media_container
                    && matches!(
                        payload.as_str(),
                ...
                {
                    *value = body_fingerprint(&serde_json::to_vec(value).unwrap_or_default());
                } else {
        ...
        Value::String(text) => {
            let is_media = text.starts_with("data:")
                || matches!(
                    normalized.as_str(),
                    "b64json" | "imagedata" | "audiodata" | "videodata" | "filedata" | "base64"
                );
            if is_media && !media {
