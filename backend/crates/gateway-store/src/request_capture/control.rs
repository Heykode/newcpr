...
impl RequestCaptureStore for CaptureManager {
    ...
    async fn stop(&self, id: &str, context: &MutationContext) -> AdminStoreResult<()> {
        ...
        let _io = self.io.lock().await;
        let mut tx = self.pool.begin().await.map_err(unavailable)?;
        let result = sqlx::query("update request_capture_tasks set task=jsonb_set(jsonb_set(task,'{status}','\"stopped\"'),'{expiresAt}',to_jsonb(least(expires_at,now()))), expires_at=least(expires_at,now()) where id::text=$1 and instance_id::text=$2")
