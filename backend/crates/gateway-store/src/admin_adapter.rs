...
impl SettingsStore for AdminSettingsStoreAdapter {
    ...
    ) -> AdminStoreResult<gateway_core::provider_ports::ProviderUserAgentOverride> {
        self.control_plane
            .load_user_agent_override(provider_kind)
            .await
            .map_err(|error| admin_store_error("outbound user-agent", error))
    }

    async fn replace_user_agent_override(
        &self,
        provider_kind: &gateway_core::routing::ProviderKind,
        selection: gateway_core::provider_ports::ProviderUserAgentOverride,
        context: &MutationContext,
        ...
        let revision = self
            ...
            .replace_user_agent_override(
                ...
                mutation_audit(
                    context,
                    "outbound_user_agent.replace",
                    "provider_outbound_user_agents",
                    provider_kind.as_str(),
                    vec!["mode".to_owned(), "custom_user_agent".to_owned()],
                ),
            )
            .await
            .map_err(|error| admin_store_error("outbound user-agent", error))?;
        ...
        let replacement = postgres::ControlPlaneReplacement {
            ...
            audit: mutation_audit(
                ...
                vec![
                    ...
                    "turn_state_probe_concurrency".to_owned(),
...
pub(crate) fn admin_runtime_settings(
    ...
    Ok(AdminRuntimeSettings {
        ...
        request_tuning: settings.request_tuning,
...
impl AuthStore for AdminAuthStoreAdapter {
    ...
    async fn consume_password_change_attempt(
        ...
        redis::AdminAuthStateRepository::consume_password_change_attempt(
            &self.state,
            admin_user_id,
            limit,
            window_seconds,
        ...
        .await
        .map_err(|error| admin_store_error("password change limit", error))
    }
...
fn store_auth_audit(event: AdminAuditModel) -> AdminStoreResult<postgres::AdminAuditEvent> {
