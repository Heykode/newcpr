...
use super::capacity_wait::{CapacityWaitCleanupWriter, RedisCapacityWait};
...
impl RedisCredentialLeaseRepository {
    ...
    pub(super) fn keys(&self, request: &CredentialLeaseRequest) -> StoreResult<[String; 3]> {
...
pub struct RedisProviderLeaseCoordinator {
    repository: RedisCredentialLeaseRepository,
    capacity_wait: RedisCapacityWait,
    process_id: String,
    sequence: AtomicU64,
}
...
impl RedisProviderLeaseCoordinator {
    #[must_use]
    pub fn new(repository: RedisCredentialLeaseRepository) -> (Self, CapacityWaitCleanupWriter) {
        let (capacity_wait, writer) =
            RedisCapacityWait::new(repository.connection.clone(), repository.clone());
        (
            Self {
                repository,
                capacity_wait,
...
impl ProviderLeasePort for RedisProviderLeaseCoordinator {
    fn try_acquire_scheduling(
        ...
        self.capacity_wait.acquire_scheduling(request)
    }
    ...
    ) -> futures::future::BoxFuture<'_, Result<ProviderWaitLeaseAcquisition, ProviderStoreError>>
    {
        self.capacity_wait.acquire(request)
    }
