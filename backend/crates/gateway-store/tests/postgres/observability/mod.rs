...
use gateway_admin::{
    ...
    ports::store::{AccountStore as _, ObservabilityStore as AdminObservabilityStore},
};
use gateway_core::{
...
#[tokio::test]
async fn usage_list_should_resolve_current_custom_names_by_account_id() {
    let Some(database) = TestDatabase::create("usage_account_custom_names").await else {
        return;
    };
    let now = Utc::now();
    seed_observability_facts(&database.pool, now)
    ...
    sqlx::query(
        "insert into provider_accounts (
           id, provider_kind, name, email, custom_name, upstream_user_id,
    ...
    for custom_name in [None, Some("个人主号"), Some("个人备用号"), None] {
        sqlx::query("update provider_accounts set custom_name = $1 where id = 'acct_observe'")
            .bind(custom_name)
            .execute(&database.pool)
            .await
            .expect("update current account custom name");
        let page = store
            ...
            .await
            .expect("usage list with current custom names");
        ...
        assert_eq!(
            team.provider_account_custom_name.as_deref(),
            Some("团队工作区")
        );
    }
    ...
    let dashboard = store
        ...
        .await
        .expect("dashboard with current account custom names");
    assert_eq!(
        dashboard
            .recent_requests
            .iter()
...
async fn usage_search_should_match_client_api_key_prefix() {
    ...
    for name in [Some("renamed key"), None] {
        ...
        let detail = observability_repository(&database.pool)
            ...
            .await
            .unwrap();
        assert_eq!(detail.request.client_api_key_name.as_deref(), name);
        assert_eq!(detail.request.client_api_key_ref, "key_observe");
        let ops = observability_repository(&database.pool)
        ...
        for item in ops
            .items
            .iter()
            .filter(|item| item.client_api_key_ref.as_deref() == Some("key_observe"))
        {
            assert_eq!(item.client_api_key_name.as_deref(), name);
        }
    }
    database.close().await;
}
...
#[tokio::test]
async fn account_health_non_completion_counts_match_diagnostics_and_scope() {
    let Some(database) = TestDatabase::create("health_non_completion").await else {
        return;
    };
    ...
    seed_observability_facts(&database.pool, now)
        ...
        .expect("seed observability facts");
    let range =
        ObservabilityRange::new(now - TimeDelta::minutes(30), now).expect("health timeline range");
    sqlx::query(
        "insert into provider_accounts (
           ...
           'acct_health_other', 'openai', 'other health account', 'user-health-other',
           'oauth', '{}'::jsonb, 1, false, true, 'ready', $1, $1, $1
         )",
    )
    .bind(now)
    .execute(&database.pool)
    .await
    .expect("seed separate account");
    sqlx::query(
        "insert into model_requests (
           id, client_api_key_ref, config_revision, protocol, operation, endpoint,
           client_transport, requested_model_id, provider_kind, provider_account_id,
           provider_account_ref, upstream_model_id, upstream_transport, attempt_count, upstream_send_state,
           outcome, cost_source, started_at, deadline_at, completed_at,
           routing_scope, routing_group_refs, routing_group_names_snapshot
         )
         select facts.id, mr.client_api_key_ref, mr.config_revision, mr.protocol,
                mr.operation, mr.endpoint, mr.client_transport, mr.requested_model_id,
                mr.provider_kind, mr.provider_account_id, mr.provider_account_ref,
                mr.upstream_model_id, mr.upstream_transport, 1, 'sent', facts.outcome, 'unavailable',
                facts.started_at, facts.started_at + interval '1 minute',
                facts.started_at + interval '1 second',
                mr.routing_scope, mr.routing_group_refs, mr.routing_group_names_snapshot
           from model_requests mr
           cross join (values
             ('req_health_cancelled', 'cancelled', $1::timestamptz),
             ('req_health_incomplete', 'incomplete', $1::timestamptz + interval '5 minutes')
           ) as facts(id, outcome, started_at)
          where mr.id = 'req_observe_failed'",
    )
    .bind(range.start)
    .execute(&database.pool)
    .await
    .expect("seed cancelled and incomplete requests");

    let account_store = admin_account_store(&database.pool);
    let repository = observability_repository(&database.pool);
    let included_start = range.start + TimeDelta::minutes(5);
    for (case, started_at, account_id, recovered, expected_non_completion) in [
        ("included", included_start, "acct_observe", false, 2),
        ("recovered", included_start, "acct_observe", true, 1),
        (
            "before range",
            range.start - TimeDelta::microseconds(1),
            "acct_observe",
            false,
            1,
        ),
        ("exclusive end", range.end, "acct_observe", false, 1),
        (
            "other account",
            included_start,
            "acct_health_other",
        ...
        let updated = sqlx::query(
            "update model_requests
                set started_at = $1, deadline_at = $1 + interval '1 minute',
                    completed_at = $1 + interval '1 second',
                    provider_account_id = $2, provider_account_ref = $2,
                    recovered_at = case when $3 then $4::timestamptz end,
                    recovery_request_id = case when $3 then 'req_observe_success' end,
                    recovery_attempt_count = case when $3 then 1 else 0 end,
                    recovery_retry_delay_ms = case when $3 then 0 end,
                    recovery_total_latency_ms = case when $3 then 1000 end
              where id = 'req_health_incomplete'",
        )
        .bind(started_at)
        .bind(account_id)
        .bind(recovered)
        ...
        .execute(&database.pool)
        .await
        .expect("set health filtering case");
        assert_eq!(updated.rows_affected(), 1, "{case}");

        let health = account_store
            .load_account_health_timeline(
                admin_observability::TimeRange::new(range.start, range.end)
                    .expect("admin health range"),
                &["acct_observe".to_owned()],
            )
            .await
            .expect("load health timeline through AccountStore");
        assert_eq!(health.len(), 1, "{case}");
        assert_eq!(health[0].account_id, "acct_observe", "{case}");
        let buckets = &health[0].request_buckets;
        assert_eq!(buckets.len(), 6, "{case}");
        let included = u64::from(expected_non_completion == 2);
        let expected = [
            (1, 0, 0, 1),
        ...
        for (index, (bucket, counts)) in buckets.iter().zip(expected).enumerate() {
            assert_eq!(
                bucket.bucket_start,
                range.start + TimeDelta::minutes(i64::try_from(index).unwrap() * 5),
                "{case}: bucket {index}"
            );
            assert_eq!(
                (
        ...
        assert_eq!(diagnostic.request_count, 4 + included, "{case}");
        assert_eq!(diagnostic.success_count, 2, "{case}");
        assert_eq!(diagnostic.failure_count, 1, "{case}");
        assert_eq!(
            diagnostic.non_completion_count, expected_non_completion,
            "{case}"
        );
        assert_eq!(
            buckets
                .iter()
                .map(|bucket| bucket.non_completion_count)
                .sum::<u64>(),
            diagnostic.non_completion_count,
            "{case}"
        );
    }
    database.close().await;
