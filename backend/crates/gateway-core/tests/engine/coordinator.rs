...
use gateway_core::error::{
    ...
    ContinuationRecoveryDisposition, GatewayError, GatewayErrorKind, OpaqueUpstreamValue,
    ProviderError, ProviderErrorKind, RawUpstreamError, StoreError, StoreErrorKind,
};
...
struct FakeStore {
    ...
    send_state_gate: Mutex<Option<oneshot::Receiver<()>>>,
...
impl ExecutionStore for FakeStore {
    ...
    async fn mark_send_state(
        ...
        let gate = self
            .send_state_gate
            .lock()
            .expect("send state gate lock")
            .take();
        if let Some(gate) = gate {
            gate.await.expect("send state released");
        }
        Ok(())
    }
        ...
        match script {
            Script::Waiting => {
                self.contexts
                    ...
                    .account_wait_budget()
                    .unwrap()
                    .enter(gateway_core::engine::AccountWaitMode::Sticky)
                    .unwrap();
                let _registration = TrackedLease(Arc::clone(&self.released_leases));
                futures::future::pending().await
            }
            Script::WaitExpired(error) => {
                self.contexts
                    .lock()
                    .unwrap()
                    .last()
...
#[test]
fn deferred_non_delivery_preserves_commit_usage_and_attempt_boundaries() {
    let operation = generate_operation();
    let route_plan = plan(&operation);
    let mut items = vec![Ok(ProviderEvent::wire(
        ProtocolWireEvent::raw_sse("openai", Bytes::from_static(b": keep-alive\n\n")).unwrap(),
    ))];
    items.extend(
        complete_stream(Some(12))
            .into_iter()
    ...
    .unwrap();
    assert!(matches!(
        session.defer_downstream_commit(),
        Err(EngineError::InvalidDeliveryState)
    ));
    let comment = block_on(session.next_event()).unwrap().unwrap();
    assert_eq!(
        comment.commit_requirement(),
        CommitRequirement::CommitBeforeDelivery
    );
    assert!(matches!(
        block_on(session.next_event()),
        Err(EngineError::DownstreamCommitRequired)
    ));
    session.defer_downstream_commit().unwrap();
    assert!(matches!(
        session.defer_downstream_commit(),
        Err(EngineError::InvalidDeliveryState)
    ));
    assert_eq!(store.state.lock().unwrap().commits, 0);
    let started = block_on(session.next_event()).unwrap().unwrap();
    assert_eq!(
        started.commit_requirement(),
        CommitRequirement::CommitBeforeDelivery
    );
    ...
    assert_eq!(state.finalizations[0].total_tokens, Some(12));
    assert_eq!(state.finalizations[0].client_status_code, Some(200));
}

#[test]
fn deferred_non_delivery_can_cancel_without_success_or_commit() {
    let operation = generate_operation();
    let route_plan = plan(&operation);
    let (coordinator, store, _) = coordinator(vec![Script::ObservedStream {
        account_id: "acct_one",
        items: vec![Ok(ProviderEvent::wire(
            ProtocolWireEvent::raw_sse("openai", Bytes::from_static(b": keep-alive\n\n")).unwrap(),
        ))],
    }]);
    let mut session = block_on(coordinator.start(
...
#[test]
fn deferred_comment_does_not_prevent_safe_account_rotation_before_commit() {
    let operation = generate_operation();
    let route_plan = plan(&operation);
    let (coordinator, store, provider) = coordinator(vec![
...
#[test]
fn completed_buffered_result_cannot_be_deferred() {
    let operation = generate_operation();
    let route_plan = plan(&operation);
...
#[test]
fn completed_stream_event_cannot_be_deferred_before_upstream_eof() {
    let operation = generate_operation();
    let route_plan = plan(&operation);
    let (coordinator, store, _) = coordinator(vec![Script::Stream {
        account_id: "acct_one",
    ...
    assert!(block_on(session.next_event()).unwrap().is_some());
    session.defer_downstream_commit().unwrap();
    let completed = block_on(session.next_event()).unwrap().unwrap();
    assert_eq!(
        completed.commit_requirement(),
        CommitRequirement::CommitBeforeDelivery
    );
    assert!(matches!(
        session.defer_downstream_commit(),
...
#[test]
fn account_wait_native_exit_respects_exact_owner_and_client_replay_dispositions() {
    for disposition in [
        ContinuationRecoveryDisposition::RetryExactConnection,
        ContinuationRecoveryDisposition::ClientReplayRequired,
    ] {
...
#[test]
fn account_wait_expired_budget_does_not_reopen_native_recovery() {
    assert_account_wait_native_exit_stops(
        ProviderError::new(
            ProviderErrorKind::AccountCapacityUnavailable,
            UpstreamSendState::NotSent,
...
fn assert_account_wait_native_exit_stops(error: ProviderError, expire: bool) {
    let Operation::Generate(generate) = generate_operation() else {
        panic!("generate");
    };
    ...
    let original = ProviderAccountId::new("acct_one").unwrap();
    ...
    let mut session = block_on(coordinator.start(
        model_request(&operation, SystemTime::now() + Duration::from_secs(60)),
        operation,
        route_plan,
        None,
    ...
    assert_eq!(
        contexts.len(),
        1,
        "account wait exits cannot create cross-owner attempts"
    );
    assert_eq!(
        contexts[0].continuation_attempt(),
        ContinuationAttempt::Native
    ...
    assert!(
        store.state.lock().unwrap().attempts.is_empty(),
        "local capacity failure is not an upstream attempt"
    );
}

#[test]
fn account_wait_context_shares_the_same_budget_across_account_retries() {
    let operation = generate_operation();
    let tuning = gateway_core::routing::RequestTuning {
        account_busy_wait_enabled: true,
        ..Default::default()
    ...
    let contexts = provider.contexts.lock().unwrap();
    assert_eq!(contexts.len(), 2);
    let first = contexts[0].account_wait_budget().unwrap();
    let second = contexts[1].account_wait_budget().unwrap();
    assert!(Arc::ptr_eq(first, second));
    assert_eq!(contexts[0].deadline(), contexts[1].deadline());
    assert_eq!(contexts[0].request_tuning(), tuning);
    first.expire();
    assert!(second.is_exhausted());
}

#[test]
fn account_wait_cancellation_drops_registration_without_sending_or_counting_attempts() {
    block_on(async {
        let operation = generate_operation();
        let route_plan = plan_with_tuning(
            &operation,
        ...
        let (coordinator, store, provider) = coordinator(vec![Script::Waiting]);
...
#[test]
fn account_wait_request_deadline_drops_registration_without_provider_failure() {
    let operation = generate_operation();
    let route_plan = plan_with_tuning(
        &operation,
        AccountSelectionPolicy::new(
    ...
    let (coordinator, _, provider) = coordinator(vec![Script::Waiting]);
    let mut session = block_on(coordinator.start(
...
#[test]
fn final_capacity_exhaustion_returns_the_local_capacity_failure() {
    let operation = generate_operation();
    let route_plan = plan(&operation);
    let raw_body = Bytes::from_static(b"{\"error\":\"stale upstream auth body\"}\x00");
    let upstream_error =
        ProviderError::new(ProviderErrorKind::Unauthorized, UpstreamSendState::NotSent)
            .with_pre_delivery_retry()
            .with_client_visible_upstream_error(ClientVisibleUpstreamError::new(
                "stale upstream auth failure",
    ...
    .expect("start execution");
    let error = block_on(session.collect_uncommitted())
        .expect_err("empty selection must return the local capacity failure");
    let EngineError::Provider(error) = error else {
        panic!("expected local capacity provider error");
    };
...
#[test]
fn final_account_exhaustion_discards_retryable_unauthorized_atomic_events() {
    for local_kind in [
        ProviderErrorKind::AccountCapacityUnavailable,
        ProviderErrorKind::NoEligibleAccount,
    ] {
        ...
        let failure =
            ProviderError::new(ProviderErrorKind::Unauthorized, UpstreamSendState::Sent)
                .with_status(401)
                .with_pre_delivery_retry()
                .with_atomic_client_events(vec![ProviderEvent::canonical_with_wire(
                vec![GatewayEvent::Started(ResponseMeta::new("stale-auth-response", "gpt-5"))],
                ProtocolWireEvent::json(
                    "openai",
                    Some("response.failed".to_owned()),
                    json!({
                        "type": "response.failed",
                        "response": {
                            "id": "stale-auth-response",
                            "status": "failed",
                            "error": {"code": "invalid_api_key", "message": "stale auth failure"}
                        }
                    }),
                )
                .expect("unauthorized failure wire"),
            )]);
        let observation = ProviderResponseObservation::new(
            UpstreamTransport::new("websocket").expect("transport"),
        )
        .with_status_code(401)
        .with_request_id(OpaqueUpstreamValue::new("stale-auth-request"));
        let (coordinator, store, provider) = coordinator(vec![
            Script::ObservedStream {
                account_id: "acct_first",
                items: vec![Ok(ProviderEvent::observation(observation)), Err(failure)],
            },
            Script::Error(ProviderError::new(local_kind, UpstreamSendState::NotSent)),
        ]);
        let mut session = block_on(coordinator.start(
            model_request(&operation, SystemTime::now() + Duration::from_secs(30)),
            operation,
        ...
        let error = block_on(session.next_event())
            .expect_err("local exhaustion must not expose retained unauthorized wire events");
        let EngineError::Provider(error) = error else {
            panic!("expected local scheduling provider error");
        };
        assert_eq!(error.kind(), local_kind);
        ...
        assert_eq!(state.commits, 0);
        assert_eq!(state.intermediate_failures, 1);
        assert_eq!(state.intermediate_status_codes, vec![Some(401)]);
        assert_eq!(
            state.intermediate_request_ids,
            vec![Some("stale-auth-request".to_owned())]
        );
        assert_eq!(state.finalizations.len(), 1);
        let finalized = &state.finalizations[0];
        assert_eq!(finalized.outcome, ExecutionOutcome::Failed);
        assert_eq!(finalized.send_state, UpstreamSendState::Sent);
        assert_eq!(finalized.attempt_count, 1);
        assert!(!finalized.committed);
        assert!(finalized.upstream_status_code.is_none());
        assert!(finalized.client_response_id.is_none());
        assert!(finalized.upstream_response_id.is_none());
        assert!(finalized.upstream_request_id.is_none());
    }
}
...
fn cancellation_before_pending_delivery_commit_reaches_terminal_state() {
    ...
    assert_eq!(
        state.finalizations[0].error_kind,
        Some(GatewayErrorKind::Cancelled)
    );
    assert!(!state.finalizations[0].committed);
    assert_eq!(session.budget_charge().amount_usd.scaled(), 0);
}
...
#[test]
fn delivery_failure_retains_upstream_facts_and_usage_without_becoming_cancellation() {
    for (buffered, committed, client_status) in [
        (true, false, 500),
        (true, false, 502),
        (false, false, 502),
        (false, true, 200),
        ...
        let observation =
            ProviderResponseObservation::new(UpstreamTransport::new("http_sse").unwrap())
                .with_status_code(200)
                .with_http_version(UpstreamHttpVersion::Http2)
                .with_request_id(OpaqueUpstreamValue::new("upstream-delivery"));
        let terminal = ProviderEvent::canonical_with_wire(
            vec![
                GatewayEvent::Started(ResponseMeta::new("response-delivery", "gpt-5")),
                GatewayEvent::Usage(Usage {
                    total_tokens: Some(21),
                    ..Usage::new()
                }),
                GatewayEvent::ProviderCost(ProviderReportedCost::from_usd_ticks(500).unwrap()),
                GatewayEvent::Completed(ResponseMeta::new("response-delivery", "gpt-5")),
            ],
            ProtocolWireEvent::json(
                "openai",
        ...
        let (coordinator, store, provider) = coordinator(vec![Script::ObservedStream {
            account_id: "acct_observed",
            items: vec![Ok(ProviderEvent::observation(observation)), Ok(terminal)],
        }]);
        let cancellation = CancellationToken::new();
        let mut session = block_on(coordinator.start(
            model_request(&operation, SystemTime::now() + Duration::from_secs(30)),
        ...
        .unwrap();
        if buffered {
            assert_eq!(block_on(session.collect_uncommitted()).unwrap().len(), 1);
        } else {
            assert!(block_on(session.next_event()).unwrap().is_some());
        }
        if committed {
            block_on(session.commit_downstream(Some(client_status))).unwrap();
        } else {
            block_on(session.record_client_status(client_status)).unwrap();
        }
        ...
        block_on(session.fail_delivery(error.clone())).unwrap();
        let charge = session.budget_charge();
        block_on(session.fail_delivery(error)).unwrap();
        block_on(session.cancel_and_finalize()).unwrap();
        assert!(block_on(session.next_event()).unwrap().is_none());
        assert!(cancellation.is_cancelled());
        assert!(session.is_finalized());
        assert_eq!(session.budget_charge().completed_at, charge.completed_at);
        assert_eq!(charge.amount_usd.scaled(), 500);
        assert!(
            session
                .provider_attempt_outcomes()
                .iter()
                .all(|outcome| !matches!(outcome, ProviderAttemptOutcome::Failed { .. })),
            "local delivery failures do not create provider health failures"
        );
        assert_eq!(provider.contexts.lock().unwrap().len(), 1);
        let state = store.state.lock().unwrap();
        assert_eq!(state.commits, usize::from(committed));
        assert_eq!(state.finalizations.len(), 1);
        let finalization = &state.finalizations[0];
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some(GatewayErrorKind::UpstreamUnavailable)
        );
        assert_eq!(finalization.client_status_code, Some(client_status));
        assert_eq!(finalization.upstream_status_code, Some(200));
        assert_eq!(
            finalization.upstream_request_id.as_deref(),
            Some("upstream-delivery")
        );
        assert_eq!(
            finalization.upstream_response_id.as_deref(),
        ...
        assert_eq!(finalization.http_version.as_deref(), Some("HTTP/2"));
        assert_eq!(finalization.send_state, UpstreamSendState::Sent);
        assert_eq!(finalization.total_tokens, Some(21));
        assert_eq!(finalization.cost_ticks, Some(500));
    }
}

#[test]
fn delivery_failure_keeps_pending_provider_error_and_raw_upstream_facts() {
    for committed in [false, true] {
        let operation = generate_operation();
        let failure = ProviderError::new(ProviderErrorKind::RateLimited, UpstreamSendState::Sent)
            .with_status(429)
            ...
            .with_atomic_client_events(vec![ProviderEvent::wire(
                ProtocolWireEvent::json(
                    "openai",
                    Some("response.failed".to_owned()),
                    json!({"type":"response.failed","response":{"id":"response-original"}}),
                )
                .unwrap(),
            )]);
        let (coordinator, store, _) = coordinator(vec![Script::ObservedStream {
        ...
        if committed {
            ...
            block_on(session.record_client_status(502)).unwrap();
        }
        block_on(session.fail_delivery(GatewayError::new(
            GatewayErrorKind::UpstreamUnavailable,
        ...
        assert_eq!(state.finalizations.len(), 1);
        let finalization = &state.finalizations[0];
        assert_eq!(finalization.error_kind, Some(GatewayErrorKind::RateLimited));
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some(if committed { 200 } else { 502 })
        );
        assert_eq!(
            finalization.outcome,
            if committed {
...
#[test]
fn delivery_failure_resumes_started_success_or_cancellation_without_replacing_it() {
    block_on(async {
        for cancelled in [false, true] {
            ...
            let mut session = coordinator
                ...
                .await
                .unwrap();
            assert_eq!(session.collect_uncommitted().await.unwrap().len(), 3);
            assert!(store.state.lock().unwrap().finalizations.is_empty());
            // Gate the terminal write under test, not an unrelated setup failure.
            *store.finalize_gate.lock().unwrap() = Some(gate);
            if cancelled {
                session.record_client_status(499).await.unwrap();
            }
            let mut original: futures::future::BoxFuture<'_, Result<(), EngineError>> = if cancelled
            {
                Box::pin(session.cancel_and_finalize())
            } else {
                Box::pin(session.commit_downstream(Some(200)))
            };
            assert!(futures::poll!(original.as_mut()).is_pending());
            drop(original);
            let mut failure = Box::pin(session.fail_delivery(GatewayError::new(
                GatewayErrorKind::UpstreamUnavailable,
                "response conversion failed",
            )));
            ...
            assert_eq!(
                finalization.error_kind,
                cancelled.then_some(GatewayErrorKind::Cancelled)
            );
            assert_eq!(
                finalization.client_status_code,
                Some(if cancelled { 499 } else { 200 })
            );
            assert_eq!(finalization.total_tokens, Some(21));
        }
    });
...
#[test]
fn interrupted_terminal_send_state_write_resumes_without_restarting_cleanup() {
    block_on(async {
        let operation = generate_operation();
        let route_plan = plan(&operation);
        ...
        session.commit_downstream(Some(200)).await.unwrap();
        let before = store.state.lock().unwrap().send_states.len();
        let (release, gate) = oneshot::channel();
        *store.send_state_gate.lock().unwrap() = Some(gate);
        session.cancel();

        let mut waiting = Box::pin(session.next_event());
        assert!(futures::poll!(waiting.as_mut()).is_pending());
        drop(waiting);
        release
            .send(())
            .expect("terminal write remains session-owned");
        session.cancel_and_finalize().await.unwrap();

        assert!(session.is_finalized());
        let state = store.state.lock().unwrap();
        assert_eq!(state.send_states.len(), before + 1);
        assert_eq!(state.finalizations.len(), 1);
        assert_eq!(state.finalizations[0].outcome, ExecutionOutcome::Cancelled);
        assert_eq!(state.finalizations[0].send_state, UpstreamSendState::Sent);
        assert!(state.finalizations[0].committed);
