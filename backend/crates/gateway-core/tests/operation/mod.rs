...
fn compact_is_explicit_and_does_not_inherit_generation_or_tools() {
    ...
    let body = Bytes::from_static(
        br#"{"model":"gpt-5.4","input":[{"type":"compaction","encrypted_content":"opaque"}]}"#,
    );
    let operation = Operation::Compact(CompactRequest::from_raw_json(
        RawJsonPayload::new("openai", body).expect("compact payload"),
    ));
    assert_eq!(operation.kind(), OperationKind::Compact);
    assert_eq!(operation.kind().as_str(), "compact");
    assert_eq!(
        operation.capability_requirements(),
        gateway_core::operation::CapabilityRequirements::new(OperationKind::Compact)
    );
    assert!(!operation.image_generation_requested());
    assert!(operation.provider_session_state("openai").is_none());
    assert!(!format!("{operation:?}").contains("opaque"));
}
