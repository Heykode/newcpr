...
/// Live admission separates temporary saturation from non-capacity rejection.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum AccountSchedulingAvailability {
    Ready,
    Busy,
...
/// Read-only limits from one consistent live publication; missing accounts fail closed.
pub trait AccountConcurrencyLimits {
    fn limit_for(&self, account_id: &str) -> Option<NonZeroU32>;
}

impl<T: AccountConcurrencyLimits + ?Sized> AccountConcurrencyLimits for Arc<T> {
    fn limit_for(&self, account_id: &str) -> Option<NonZeroU32> {
        self.as_ref().limit_for(account_id)
    }
}
...
impl AccountSelector {
    ...
    pub fn availability(
        ...
        limits: &dyn AccountConcurrencyLimits,
    ) -> AccountSchedulingAvailability {
        if let Some(blocker) = self.non_capacity_blocker(candidate, context) {
            return AccountSchedulingAvailability::Blocked(blocker);
        }
        let Some(limit) = limits.limit_for(candidate.account.id().as_str()) else {
            return AccountSchedulingAvailability::Blocked(
    ...
    #[must_use]
    pub fn select_with_live_capacity<'a>(
        &self,
    ...
    /// Selecting a busy candidate does not prove all ready candidates were tried.
    #[must_use]
    pub fn select_for_capacity_wait<'a>(
        &self,
        candidates: &'a [AccountCandidate],
        context: &AccountSelectionContext,
        limits: &dyn AccountConcurrencyLimits,
    ...
    fn select_live<'a>(
        ...
        context: &AccountSelectionContext,
        limits: &dyn AccountConcurrencyLimits,
        expected: AccountSchedulingAvailability,
    ) -> Option<AccountSelection<'a>> {
        self.select_ranked(
            candidates,
            context,
            |candidate| match self.availability(candidate, context, limits) {
                available if available == expected => None,
                AccountSchedulingAvailability::Blocked(blocker) => Some(blocker),
                _ => Some(AccountSchedulingBlocker::ConcurrencyLimit),
            },
            |candidate| {
    ...
    #[must_use]
    pub fn capacity_snapshot_with_live_limits(
        &self,
        ...
        let (used_slots, total_slots) = candidates
            .iter()
            .filter(|candidate| self.non_capacity_blocker(candidate, context).is_none())
            ...
            .fold((0_u64, 0_u64), |(used, total), (in_flight, limit)| {
                (used.saturating_add(in_flight), total.saturating_add(limit))
            });
    ...
    fn select_ranked<'a>(
        ...
        candidates: &'a [AccountCandidate],
        context: &AccountSelectionContext,
        blocker: impl Fn(&AccountCandidate) -> Option<AccountSchedulingBlocker>,
        limit: impl Fn(&AccountCandidate) -> NonZeroU32 + Copy,
    ) -> Option<AccountSelection<'a>> {
        let mut eligible = candidates
            .iter()
            .filter(|candidate| blocker(candidate).is_none())
            .collect::<Vec<_>>();
        ...
        let preferred = if let Some(preferred) = context.preferred_account.as_ref() {
            ...
            {
                Some(candidate) => match blocker(candidate) {
            ...
            RotationStrategy::QuotaResetPriority => {
                eligible.sort_by(|left, right| {
                    (
                        ...
                        .then_with(|| {
                            utilization_at_limit(left, limit(left))
                                .total_cmp(&utilization_at_limit(right, limit(right)))
                        })
                        .then_with(|| {
                            left.signals
                                .last_started_at
...
fn utilization_at_limit(candidate: &AccountCandidate, limit: NonZeroU32) -> f64 {
