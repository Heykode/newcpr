...
use super::{
    ...
    ProviderAccountUpdate, ProviderDeviceCodec, ProviderRefreshQuery, QuotaAccessChange,
    QuotaObservation, QuotaObservationTouch, QuotaWriteOutcome,
};
...
pub trait ProviderAccountStore: Send + Sync {
    ...
    /// Apply a provider health observation for an explicitly selected account.
    ///
    /// Unlike ordinary state changes, diagnostics may update a disabled account,
    /// but they must never change its enabled flag. Implementations must retain
    /// the same revision and observation-time fencing as `apply_state_change`.
    async fn apply_diagnostic_state_change(
        &self,
