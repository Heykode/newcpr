...
pub fn resolve_account_status(
    ...
    resolve_account_operational_status(facts, now)
}

/// 管理页面的运行状态不被启停遮盖；调度仍使用包含禁用屏障的状态解析器。
#[must_use]
pub fn resolve_account_operational_status(
    facts: &AccountStatusFacts,
    now: SystemTime,
) -> AccountStatusProjection {
...
/// The provider alone understands credential JSON. Store transactions invoke
/// this codec to project or restore the stable device fact without archiving
/// authentication material.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ProviderDeviceCodecError {
    Invalid,
    Conflict,
    Unavailable,
}

pub trait ProviderDeviceCodec: Send + Sync {
...
impl CredentialCasUpdate {
    ...
    /// Preserve the profile current at commit time during token-only refresh.
    #[must_use]
...
pub struct QuotaObservation {
    ...
    /// Only a fresh explicit provider observation may change the plan.
    pub plan_type: Option<String>,
