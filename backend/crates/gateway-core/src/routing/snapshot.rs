...
use crate::runtime::AccountConcurrencySnapshot;
...
async fn compile_runtime_snapshot(
    ...
    let default_concurrency = NonZeroU32::new(facts.settings.max_concurrent_per_account)
        .ok_or(RuntimeSnapshotCompileError::InvalidData)?;
    let mut account_limits = BTreeMap::new();
    for account in facts.provider_accounts {
        let limit = match account.concurrency_limit {
            Some(limit) => {
                NonZeroU32::new(limit).ok_or(RuntimeSnapshotCompileError::InvalidData)?
            }
            None => default_concurrency,
        };
        account_limits.insert(account.account_id.as_str().to_owned(), limit);
...
impl RuntimeSnapshot {
    ...
    #[must_use]
    pub fn with_account_concurrency_limits(mut self, limits: BTreeMap<String, NonZeroU32>) -> Self {
        self.account_concurrency = Arc::new(AccountConcurrencySnapshot::new(self.revision, limits));
        self
    }

    #[must_use]
    pub fn account_concurrency(&self) -> Arc<AccountConcurrencySnapshot> {
        Arc::clone(&self.account_concurrency)
    }
