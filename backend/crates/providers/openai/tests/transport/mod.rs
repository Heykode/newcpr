...
async fn collect_backend_response(
    ...
    if let Some(updates) = response_metadata_updates {
        let updates = updates.lock().await;
        turn_state = updates.turn_state.clone().or(turn_state);
        reported_model = updates.reported_model.clone().or(reported_model);
    }
...
async fn execute_response_create_request(
    ...
    let upstream_request = websocket_only_request(upstream_request);
...
fn websocket_only_request(mut request: CodexResponsesRequest) -> CodexResponsesRequest {
    // Event/pool fixtures do not serve HTTP. Keep their opening independent of
    // the optional 800ms fast path without changing the upstream wire body.
    if request.previous_response_id().is_some() {
        request.previous_response_scope = Some(PreviousResponseScope::ExternalUnknown);
    } else {
        assert!(
            !request.store(),
            "WS-only new-chain fixtures must retain their socket"
        );
        request
            .local_conversation_id
            .get_or_insert_with(|| "websocket-fixture".to_owned());
        request
            .downstream_websocket_connection_id
            .get_or_insert_with(|| "websocket-fixture-lane".to_owned());
    }
