...
async fn backend_http_should_zstd_compress_codex_responses_request_body() {
    ...
    let instructions = "compress me ".repeat(128);
    let mut request = codex_request("gpt-test", instructions.clone(), Vec::new());
    request.force_http_sse = true;
    let client = CodexBackendClient::new(
        reqwest::Client::builder()
            .no_proxy()
...
async fn backend_http_should_force_upstream_sse_for_non_streaming_client_request() {
    ...
    assert_eq!(
        read_header_value(head, "authorization"),
        Some("Bearer access-token")
    );
    assert_eq!(
        read_header_value(head, "chatgpt-account-id"),
        Some("acct-non-streaming")
    );
    assert_eq!(
        read_header_value(head, "session-id"),
