...
fn decoder_should_bill_the_sent_model_independently_of_the_response_model() {
    for (sent, returned, expected_cost) in [
        ...
        ("gpt-6-future", Some("gpt-6-sol"), None),
    ] {
        let created =
...
#[test]
fn decoder_should_use_requested_service_tier_for_billing() {
    let body = concat!(
        "event: response.created\n",
...
#[test]
fn decoder_should_use_default_billing_when_request_omits_tier() {
    let body = concat!(
        "event: response.created\n",
...
fn gpt6_decoder_should_use_reasoning_preview_web_search_price() {
    for model in ["gpt-6-astra", "gpt-6-sol", "gpt-6-luna"] {
        ...
        for tool_type in [
            ...
            let tools = vec![json!({ "type": tool_type })];
...
#[test]
fn websocket_decoder_should_use_requested_service_tier_for_billing() {
...
fn billing_should_add_file_and_web_search_fees_without_tier_markup() {
    ...
    for (tier, expected) in [("default", 1_150_000_000), ("fast", 1_150_000_000)] {
        assert_eq!(
