...
use gateway_core::account::{
    AccountErrorReason, AccountFeedbackStats, AccountWeight, CredentialState, OpaqueProviderData,
...
fn provider_and_quota_with_profile(
    store: &Arc<MemoryAccountStore>,
    session_affinity: Arc<MemorySessionAffinity>,
    base_url: String,
    leases: Arc<TestLeaseCoordinator>,
...
fn websocket_fixture_request(request: GenerateRequest) -> GenerateRequest {
    let payload = request.protocol_payload();
    let mut context = payload.context().clone();
    context.insert(
        "downstream_websocket_connection_id".to_owned(),
        json!("provider-websocket-fixture"),
    );
    GenerateRequest::from_protocol_payload(payload.clone().with_context(context))
        .with_provider_session_state(
            request
                .provider_session_state("openai")
                .expect("WS fixture account and conversation")
                .clone(),
        )
}

#[test]
fn required_websocket_fixture_preserves_body_and_account_session() {
    let request = generate_with_persisted_session_context(
        "acct_fixture",
        "conversation",
        "session",
        "thread",
    );
    let required = websocket_fixture_request(request.clone());
    assert_eq!(
        required.protocol_payload().body(),
        request.protocol_payload().body()
    );
    assert_eq!(
        required.provider_session_state("openai"),
        request.provider_session_state("openai")
    );
}
...
async fn capture_turn_state_request(
    ...
    capture_turn_state_request_with_mirrors(
        request_id,
        previous_turn_id,
        current_turn_id,
        client_turn_state,
...
async fn capture_turn_state_request_with_mirrors(
    request_id: &str,
    previous_turn_id: Option<&str>,
    current_turn_id: Option<&str>,
    client_turn_state: Option<&str>,
    ...
    if mirrors {
        let state = client_turn_state.unwrap_or("stale-mirror");
        let turn_metadata = format!(
            r#"{{"turn_state":{},"thread_id":"thread","keep":"business","workspace":"/tmp/\u4e2d\u6587/\ud83d\ude80"}}"#,
            json!(state)
        );
        protocol_context.insert(
            "opaque_request_headers".to_owned(),
            json!([
                ["x-codex-turn-state", STANDARD.encode(state)],
                ["x-codex-turn-metadata", STANDARD.encode(&turn_metadata)]
            ]),
        );
        body.insert("turn_state".to_owned(), json!(state));
        body.insert("turn_metadata".to_owned(), json!(turn_metadata));
        body.insert(
            "client_metadata".to_owned(),
            json!({"x-codex-turn-state": state, "turnState": state, "keep": "business",
...
async fn websocket_close_after_delivery_preserves_details_and_reconnects_through_pool() {
    ...
    assert_eq!(raw["reason"], "message too big");
    assert_eq!(
...
async fn cross_account_device_header_and_body_use_the_selected_stored_identity() {
    ...
    let old = repository
        .load_runtime_credential(&store.account("acct_scope_old").unwrap())
        .await
        .unwrap();
    let selected = repository
        .load_runtime_credential(&store.account("acct_scope_new").unwrap())
        .await
        .unwrap();
    assert_ne!(old.installation_id, selected.installation_id);
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/codex/responses"))
        .respond_with(
    ...
    let payload = ProtocolPayload::json_object(
        ...
        json!({
            "model": "gpt-5.4",
            "input": "hello",
            "installation_id": old.installation_id.as_str(),
            "client_metadata": {
                "installation_id": old.installation_id.as_str(),
                "x-codex-installation-id": old.installation_id.as_str()
            }
        })
        .as_object()
        .unwrap()
    ...
    .with_context(Map::from_iter([
        ...
        (
            ...
            json!([[
                ...
                STANDARD.encode(&old.installation_id)
            ]]),
        ),
    ]));
    let mut stream = provider_with_base_url(&store, server.uri())
        .execute(
            ...
            context_with_state_owner("req_device_switch", "acct_scope_old"),
        )
        .await
        .unwrap();
    while let Some(event) = stream.next().await {
    ...
    for pointer in [
        ...
        assert_eq!(
            ...
            Some(selected.installation_id.as_str()),
            "{pointer}"
        );
    }
}
    ...
    for owner in ["acct_scope_same", "acct_scope_old"] {
        ...
        let request = capture_scoped_http_request(
            ...
            Map::from_iter([
                ...
                (
                    "opaque_request_headers".to_owned(),
                    json!([["x-codex-turn-metadata", STANDARD.encode(raw)]]),
                ),
            ]),
        )
        .await;
...
async fn account_selection_log_should_include_affinity_observation_fields() {
    ...
    let request_id = "req_supplementary_affinity_observation";
    let operation = Operation::Generate(GenerateRequest::from_protocol_payload(
        ProtocolPayload::json_object(
            "openai",
        ...
        .with_context(Map::from_iter([(
            ...
            gateway_protocol::openai::OpenAiSchedulingSessionHint::new(
                "x-session-id",
                "synthetic-routing-only",
            )
            .unwrap()
            .to_context_value(),
        )])),
    ...
    let stream = provider
        ...
        .await
        .expect("prepare supplementary observation");
    drop(stream);
    let events = captured.json_events();
    let supplementary = selected_account_log_fields(&events, request_id);
    assert_eq!(
        supplementary["affinity_anchor_source"],
        "supplementary-session"
    );
    assert_eq!(
        supplementary["affinity_anchor"],
        supplementary["affinity_key_hash"]
    );
    assert_eq!(supplementary["session_id"], "");
    assert_eq!(supplementary["session_id_present"], false);
    assert!(
        !serde_json::to_string(supplementary)
            .unwrap()
            .contains("synthetic-routing-only")
    );
}
...
#[tokio::test]
async fn disabled_account_diagnostic_success_uses_upstream_and_persists_health() {
    let store = Arc::new(MemoryAccountStore::default());
...
#[tokio::test]
async fn disabled_account_diagnostic_failure_uses_upstream_and_persists_health() {
    let store = Arc::new(MemoryAccountStore::default());
    let account_id = "acct_disabled_diagnostic_failure";
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(
            ResponseTemplate::new(401)
                .insert_header("content-type", "application/json")
                .set_body_string(
                    r#"{"error":{"code":"invalid_api_key","message":"credential rejected"}}"#,
                ),
        )
        .expect(1)
        .mount(&server)
    ...
    let (provider, _) = provider_and_quota_with_affinity_and_base_url_and_leases(
        &store,
        Arc::new(MemorySessionAffinity::default()),
        server.uri(),
        Arc::new(TestLeaseCoordinator::default()),
    ...
    let mut stream = provider
        .execute(
            planned_request("openai", http_generate_operation()),
            diagnostic_context("req_disabled_diagnostic_failure", account_id),
        )
        .await
        .expect("disabled diagnostic should prepare a fixed-account stream");
    ...
    assert!(failed, "diagnostic failure should reach the caller");

    let account = store
        .account(account_id)
        .expect("disabled test account after failure");
    assert!(!account.enabled());
    ...
    assert_eq!(account.last_error_message(), Some("credential rejected"));
    assert!(!store.has_quota(account_id));
}

#[tokio::test]
async fn diagnostic_quota_failures_do_not_schedule_authoritative_usage_refresh() {
    for (diagnostic, code, status, evidence) in [
        (
            true,
            "insufficient_quota",
            402,
        ...
        store
            ...
            .await
            .expect("seed display baseline");
        let server = MockServer::start().await;
        Mock::given(method("POST"))
            .and(path("/codex/responses"))
            .respond_with(ResponseTemplate::new(status).set_body_json(json!({
                "error": {"code": code, "type": code, "message": "quota fixture"}
            })))
            .expect(1)
            .mount(&server)
            .await;
        Mock::given(method("GET"))
            ...
            .respond_with(ResponseTemplate::new(200).set_body_json(json!({
                "rate_limit": {
                    "allowed": true,
                    "limit_reached": false,
                    "primary_window": {"used_percent": 0, "reset_at": 1_900_018_000}
                }
            })))
            .expect(u64::from(!diagnostic))
            .mount(&server)
            .await;
        let (provider, quota) = provider_and_quota_with_affinity_and_base_url_and_leases(
            &store,
        ...
        let attempt = if diagnostic {
            diagnostic_context("req_diagnostic_quota_refresh", account_id)
        } else {
            context("req_ordinary_quota_refresh", CancellationToken::new())
        };
        let mut stream = provider
            .execute(
                planned_request("openai", http_generate_operation()),
            ...
            .await
            .expect("prepare quota failure");
        let failures = timeout(Duration::from_secs(5), async {
        ...
        .await
        .expect("quota failure must finish independently of usage refresh");
        assert_eq!(failures.len(), 1, "diagnostic={diagnostic}, code={code}");
        assert_eq!(failures[0].kind(), ProviderErrorKind::QuotaExhausted);
        assert_eq!(failures[0].upstream_status(), Some(status));
        let rejected = store.account(account_id).expect("rejected account");
        assert_eq!(rejected.quota().access(), QuotaAccessState::Exhausted);
        assert_eq!(rejected.quota().evidence(), Some(evidence));
        let failure_observed_at = quota
            .read_account(account.id())
            .await
            .expect("read rejected quota")
            .expect("display baseline")
            .observed_at();
        drop(stream);
        drop(provider);
        if diagnostic {
            // A scheduled follow-up owns an Arc even during its settlement delay.
            // Reclaiming the service fences future work, not just current HTTP count.
            assert!(
                Arc::try_unwrap(quota).is_ok(),
                "diagnostic {code} must leave no background quota-refresh owner"
            );
        } else {
            let pending_refresh = Arc::downgrade(&quota);
            drop(quota);
            timeout(Duration::from_secs(8), async {
                while pending_refresh.strong_count() != 0 {
                    tokio::time::sleep(Duration::from_millis(10)).await;
                }
            })
            .await
            .expect("ordinary failure refresh must finish and release its service");
        }
        let observation = store
            .get_quotas(std::slice::from_ref(account.id()))
            .await
            .expect("settled quota")
            .pop()
            .expect("stored observation");
        if diagnostic {
            assert_eq!(observation.observed_at, failure_observed_at);
        } else {
            assert!(observation.observed_at > failure_observed_at);
        }
        assert_eq!(observation.state.access(), QuotaAccessState::Exhausted);
        assert_eq!(Value::Object(observation.quota.into_inner()), raw_quota);
        let requests = server.received_requests().await.expect("settled requests");
        assert_eq!(
            requests
                .iter()
                .filter(|request| request.url.path() == "/api/codex/usage")
            ...
            "checked after the quota-refresh ownership fence"
        );
        server.verify().await;
    }
}
...
async fn provider_preserves_independent_catalog_context_windows() {
    for (context_window, max_context_window) in [
        ...
        assert_eq!(capabilities.len(), 1);
        assert_eq!(capabilities[0].upstream_model().as_str(), "gpt-5.6-terra");
