...
#[tokio::test]
async fn oauth_import_rejects_conflicting_token_principals_before_merging_metadata() {
    let service = CodexCredentialAdminService::new(
        Arc::new(UnusedRefresher),
        Arc::new(TestLeaseCoordinator::default()),
        runtime_policy(),
    );
    for (id_user, id_account, at_user, at_account) in [
        (Some("user-A"), Some("A"), Some("user-B"), Some("A")),
        (Some("user-A"), Some("A"), Some("user-A"), Some("B")),
        (Some("user-A"), None, Some("user-B"), Some("B")),
        ...
        let error = service
            .prepare_import_document(serde_json::json!({
                "accessToken": token(at_user, at_account),
                "idToken": token(id_user, id_account),
            }))
            .await
            .expect_err("different principals must not be merged");
        assert_eq!(
            error,
            provider_openai::credential::CodexCredentialAdminError::PrincipalConflict
        );
    }
}

#[tokio::test]
async fn oauth_import_opaque_and_blank_identity_remain_unresolved() {
    let service = CodexCredentialAdminService::new(
        Arc::new(UnusedRefresher),
        Arc::new(TestLeaseCoordinator::default()),
        runtime_policy(),
    ...
    let prepared = service
        .prepare_import_document(serde_json::json!({
            "accessToken": "opaque-new-token",
            "idToken": test_jwt(serde_json::json!({
                "https://api.openai.com/auth": {
        ...
        .await
        .expect("unresolved import remains supported");
