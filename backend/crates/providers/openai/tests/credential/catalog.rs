...
pub(super) const OFFICIAL_FIXTURE: &[u8] =
...
pub(super) fn service_with_catalog_cache(
...
pub(super) fn client_scope(accounts: &[ProviderAccount]) -> FrozenAccountScope {
...
#[tokio::test]
async fn plan_catalog_preserves_upstream_status_for_admin_diagnostics() {
    let store = Arc::new(MemoryAccountStore::default());
    let account = seed_account(&store, "acct_catalog_unauthorized").await;
    let server = MockServer::start().await;
    Mock::given(method("GET"))
        .and(path("/codex/models"))
        .respond_with(
    ...
    let error = service
        .refresh_account_catalog(account.id())
        .await
        .expect_err("unauthorized catalog request");

    match error {
        CodexCredentialCatalogError::Upstream { status, .. } => assert_eq!(status, Some(401)),
