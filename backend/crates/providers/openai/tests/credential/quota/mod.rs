...
async fn global_qx_service(
    server: &MockServer,
) -> (Arc<MemoryAccountStore>, CodexCredentialQuotaService, String) {
    let store = Arc::new(MemoryAccountStore::default());
    create_account(&store, "acct_qx_desktop").await;
...
fn assert_desktop_account_request(request: &wiremock::Request, desktop_ua: &str) {
    assert_eq!(request.headers["user-agent"], desktop_ua);
    assert_eq!(
        request.headers["authorization"],
        "Bearer token-acct_qx_desktop"
    );
    assert_eq!(
        request.headers["chatgpt-account-id"],
        "chatgpt-acct_qx_desktop"
    );
    for name in [
        "cookie",
        "originator",
...
#[tokio::test]
async fn quota_refresh_global_qx_preserves_desktop() {
    let server = MockServer::start().await;
    Mock::given(method("GET"))
        .and(path("/backend-api/wham/usage"))
        .respond_with(
            ResponseTemplate::new(200)
                .insert_header("set-cookie", "__cf_bm=synthetic-quota-cookie; Path=/")
                .set_body_json(json!({
                    "rate_limit": {
                        "allowed": true,
        ...
        .mount(&server)
        .await;
    let (store, service, desktop_ua) = global_qx_service(&server).await;
    let account = store.account("acct_qx_desktop").expect("account");

    // A second service refresh must not replay the auxiliary response's Cookie.
    for _ in 0..2 {
        let snapshot = service
            .refresh_account(account.id())
            .await
            .expect("quota refresh");
        assert_eq!(snapshot.fact().remaining_percent(), Some(92));
    }
    let requests = server.received_requests().await.expect("requests");
    assert_eq!(requests.len(), 2);
    for request in &requests {
        assert_desktop_account_request(request, &desktop_ua);
    }
    server.verify().await;
}

#[tokio::test]
async fn reset_credits_global_qx_preserves_desktop() {
    let server = MockServer::start().await;
    Mock::given(method("GET"))
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
            "code": "reset",
            "credit": {"id": "credit_1", "reset_type": "codex_rate_limits"}
        })))
        .expect(1)
        .mount(&server)
        .await;
    ...
    let account = store.account("acct_qx_desktop").expect("account");
    let credits = service
        .list_reset_credits(account.id())
        .await
        .expect("list");
    assert_eq!(credits.available_count, 1);
    let redeem_request_id =
        uuid::Uuid::parse_str("8fbf302d-11df-4bd5-82e4-08e4b3df7874").expect("UUID");
    let result = service
        .consume_reset_credit(account.id(), Some("credit_1"), redeem_request_id)
        .await
        .expect("consume");
    ...
    assert_eq!(
        serde_json::from_slice::<serde_json::Value>(&requests[1].body).expect("consume body"),
        json!({"credit_id": "credit_1", "redeem_request_id": redeem_request_id.to_string()})
    );
    server.verify().await;
}
