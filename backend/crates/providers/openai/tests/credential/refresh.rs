...
struct RefreshEgressStore {
    loads: AtomicUsize,
    fail_after_initial_load: bool,
}

impl gateway_core::provider_ports::egress::ProviderEgressStorePort for RefreshEgressStore {
    fn load(
        &self,
    ) -> BoxFuture<
        '_,
        Result<Arc<gateway_core::provider_ports::egress::ProviderEgressConfig>, ProviderStoreError>,
    > {
        Box::pin(async {
            let count = self.loads.fetch_add(1, Ordering::SeqCst);
            if count > 0 && self.fail_after_initial_load {
    ...
    fn ensure_fixed_affinity(
        &self,
        _: &ProviderAccountId,
    ) -> BoxFuture<'_, Result<Option<std::net::Ipv6Addr>, ProviderStoreError>> {
        Box::pin(async { panic!("refresh service must not allocate affinity itself") })
    }
}

#[tokio::test]
async fn successful_refresh_reloads_egress_without_turning_publication_failure_into_token_failure()
{
    for fail_after_initial_load in [false, true] {
        let store = Arc::new(MemoryAccountStore::default());
        let egress_store = Arc::new(RefreshEgressStore {
            loads: AtomicUsize::new(0),
            fail_after_initial_load,
        });
        let runtime =
            provider_openai::transport::egress::CodexEgressRuntime::load(egress_store.clone())
                .await
                .expect("initial egress load");
        let refresher = SingleUseRefresher::new();
        let service = refresh_service(
            &store,
            Arc::clone(&refresher),
        ...
        let account_id = "acct_refresh_egress_publication";
        seed_refreshable_account(
            &store,
            account_id,
        ...
        let outcomes = service.refresh_due().await.expect("credential refresh");
        ...
        assert_eq!(refresher.calls(), 1);
        assert_eq!(egress_store.loads.load(Ordering::SeqCst), 2);
        let account = store.account(account_id).expect("refreshed account");
        let loaded = store
            .load_credential(account.id(), account.revision())
            .await
            .expect("persisted rotated credential");
        let credential = CodexCredentialCodec::decode(&loaded.credential).unwrap();
        assert_eq!(
            credential
                .authentication
                .oauth()
