...
#[tokio::test]
async fn profile_statistics_global_qx_preserves_desktop() {
    ...
    Mock::given(method("GET"))
        ...
        .mount(&server)
        .await;
    let (store, _) = service("acct_qx_profile", &server).await;
    ...
    let account = store.account("acct_qx_profile").expect("account");
    let statistics = service
        .profile_statistics(account.id())
        .await
        .expect("statistics");
    assert_eq!(statistics.display_name.as_deref(), Some("Ada"));
    assert_eq!(statistics.summary.total_text_tokens, Some(42));
    ...
    assert_eq!(
        ...
        "Bearer token-acct_qx_profile"
    );
    assert_eq!(
        request.headers["chatgpt-account-id"],
        "chatgpt-acct_qx_profile"
    );
    for name in [
        "cookie",
        "originator",
