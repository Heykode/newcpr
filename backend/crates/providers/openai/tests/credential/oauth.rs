...
struct CandidateExchanger {
    access_token: String,
    id_token: String,
}

#[async_trait]
impl AuthorizationCodeExchanger for CandidateExchanger {
    async fn exchange_authorization_code(
        &self,
        _grant: AuthorizationCodeGrant,
    ) -> Result<AuthorizationTokenSet, AuthorizationCodeExchangeError> {
        Ok(AuthorizationTokenSet {
            secret: CodexOAuthSecret {
                access_token: SecretString::from(self.access_token.clone()),
                refresh_token: None,
                id_token: None,
            },
            id_token: SecretString::from(self.id_token.clone()),
...
#[tokio::test]
async fn reauthorization_checks_candidate_before_reusing_a_missing_refresh_token() {
    ...
    for same_user in [false, true] {
        ...
        store
            .seed_oauth_credential(ImportCodexOAuthCredential {
                account_id: "acct_fallback_rt".to_owned(),
                name: "fallback RT".to_owned(),
                secret: secret("old-access"),
                verified_account: account_profile("A"),
                next_refresh_at: None,
                enabled: true,
            })
            .await;
        let account_id = ProviderAccountId::new("acct_fallback_rt").unwrap();
        let before = store.load_current_credential(&account_id).await.unwrap();
        let pending = Arc::new(PendingStore::default());
        let service = CodexOAuthAdminService::new(
            pending.clone(),
        ...
        let started = service
            .start_authorization(StartCodexOAuthAuthorization {
                mutation: reauthorization_mutation(account_id.clone(), "fallback-rt"),
            })
            .await
            .unwrap();
        let state = pending
        ...
        if same_user {
            ...
            assert_eq!(prepared.profile.email.as_deref(), Some("A@example.com"));
            assert_eq!(prepared.profile.plan_type.as_deref(), Some("pro"));
            let credential = prepared.credential.into_parts().credential;
            let runtime = CodexCredentialCodec::decode(&credential).unwrap();
            assert_eq!(
                runtime
                    .authentication
                    .oauth()
...
#[tokio::test]
async fn first_exchange_rejects_conflicting_id_and_access_principals() {
    use secrecy::ExposeSecret;
    let pending = Arc::new(PendingStore::default());
    let service = CodexOAuthAdminService::new(
        pending.clone(),
        Arc::new(CandidateExchanger {
            access_token: id_token(serde_json::json!({
    ...
    let error = service
        ...
        .await
        .unwrap_err();
    assert_eq!(error, CodexOAuthAdminError::PrincipalConflict);
}
