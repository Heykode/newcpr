...
impl ProviderAccountStore for MemoryAccountStore {
    ...
    async fn get_account(
        ...
        if self.pause_account_read.load(Ordering::SeqCst) {
    ...
    async fn list_for_provider(
        ...
        let hook = self.before_provider_list.lock().unwrap().take();
...
#[derive(Default)]
pub(crate) struct TestCapacityState {
    ...
    pub(crate) fail_signals: AtomicBool,
    pub(crate) fail_wait: AtomicBool,
    pub(crate) expire: AtomicBool,
    pub(crate) pause_signals: AtomicBool,
    pub(crate) race_busy: AtomicBool,
    pub(crate) pause_acquire_reply: AtomicBool,
    pub(crate) safe_acquires: AtomicUsize,
    pub(crate) after_acquire: Mutex<Option<Box<dyn FnOnce() + Send + Sync>>>,
    pub(crate) after_state_load: Mutex<Option<Box<dyn FnOnce() + Send + Sync>>>,
}

impl TestCapacityState {
    pub(crate) fn set_load(&self, id: &str, in_flight: u32) {
        self.signals
            .lock()
            .unwrap()
            .entry(ProviderAccountId::new(id).unwrap())
            .or_insert_with(empty_capacity_signals)
            .in_flight = in_flight;
    }
    ...
    fn acquire(
    ...
    ) -> Option<Box<dyn ProviderLeaseGuard>> {
        let mut signals = self.signals.lock().unwrap();
        let signal = signals
            .entry(request.account_id().clone())
            .or_insert_with(empty_capacity_signals);
        if self.race_busy.load(Ordering::SeqCst)
            || signal.in_flight >= request.max_concurrent().get()
            || signal.last_started_at.is_some_and(|at| {
        ...
        signal.in_flight += 1;
        signal.last_started_at = Some(SystemTime::now());
        drop(signals);
        if let Some(hook) = self.after_acquire.lock().unwrap().take() {
            hook();
        }
        Some(Box::new(TestCapacityExecution {
...
fn empty_capacity_signals() -> AccountRuntimeSignals {
...
struct TestCapacityExecution {
    state: Arc<TestCapacityState>,
    id: ProviderAccountId,
}
impl Drop for TestCapacityExecution {
    fn drop(&mut self) {
        let mut signals = self.state.signals.lock().unwrap();
        if let Some(signal) = signals.get_mut(&self.id) {
            signal.in_flight = signal.in_flight.saturating_sub(1);
        }
    }
}

struct TestCapacityWait {
    state: Arc<TestCapacityState>,
    active: bool,
}
impl Drop for TestCapacityWait {
    fn drop(&mut self) {
        if self.active {
...
impl ProviderWaitLease for TestCapacityWait {
    fn try_promote(
        &mut self,
        request: ProviderSchedulingLeaseRequest,
        ...
        Box::pin(async move {
            ...
            if let Some(guard) = self.state.acquire(&request) {
                self.state.waiting.fetch_sub(1, Ordering::SeqCst);
                self.active = false;
                Ok(ProviderWaitPromotion::Acquired(guard))
            } else {
                Ok(ProviderWaitPromotion::Busy { retry_after: None })
            }
        })
    }
                ...
                .map(|id| {
                    (
                        ...
                        signals
                            .get(id)
                            .cloned()
                            .unwrap_or_else(empty_capacity_signals),
                    )
                })
                .collect())
        })
...
impl MemorySessionAffinity {
    ...
    pub(crate) fn fail_next_lookup(&self) {
...
pub(crate) struct MemorySessionExclusions {
    ...
    pub(crate) lookups: Mutex<Vec<ProviderSessionAffinityKey>>,
