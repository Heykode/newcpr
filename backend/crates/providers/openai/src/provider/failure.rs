...
pub(super) async fn apply_failure(
    ...
    if let Some(account_failure) = failure.account_failure {
        ...
        let result = if context.is_diagnostic {
            context
                .selector
                .record_diagnostic_failure(account, account_failure, failure.error_message.clone())
                .await
        } else {
            context
                .selector
...
fn should_schedule_authoritative_quota_refresh(
...
pub(super) struct WebSocketRecoveryContext<'a> {
    ...
    pub(super) allow_http_fallback: bool,
...
pub(super) fn apply_websocket_recovery_policy(
    ...
    if !context.allow_http_fallback || !context.requirement.allows_pre_send_http_fallback() {
        return;
    }
        ...
        CodexClientError::WebSocket(error) => {
            ...
            if close_code == Some(1009) {
                failure.error = failure
                    .error
                    .with_status(413)
                    .with_upstream_code(OpaqueUpstreamValue::new("message_too_big".to_owned()));
            } else if let Some(close_code) = close_code {
                failure.error =
                    failure
                        .error
            ...
            if let Some(client_visible_error) = client_visible_error {
                if error.is_message_too_big() {
                    failure.error = failure.error.with_client_visible_upstream_response(
                        ClientVisibleUpstreamResponse::new(
                            413,
                            Some(b"application/json".to_vec()),
                            Bytes::from(
                                json!({
                                    "error": {
                                        "message": client_visible_error.message(),
                                        "type": client_visible_error.error_type(),
                                        "code": client_visible_error.code()
                                    }
                                })
                                .to_string(),
                            ),
...
fn client_diagnostic(error: &CodexClientError) -> Option<ProviderDiagnostic> {
    let (stage, code, message) = match error {
        ...
        CodexClientError::Egress(error) => {
            ("prepare", "local_egress_unavailable", error.to_string())
        }
        CodexClientError::ModelCatalog(_) => (
            "prepare",
...
fn websocket_diagnostic_message(error: &CodexWebSocketExchangeError) -> ProviderDiagnostic {
    ...
    ProviderDiagnostic::new(match error.diagnostic_cause() {
        ...
        CodexWebSocketExchangeError::Egress(error) => {
            format!("OpenAI local egress is unavailable: {error}")
        }
