...
impl CodexProvider {
    ...
    pub(super) async fn execute_raw_json_endpoint(
        ...
        let turn_metadata = request.turn_metadata.as_deref().and_then(|metadata| {
            crate::transport::request::scope_raw_turn_metadata(metadata, lease.installation_id())
        });
        let body = crate::transport::request::scope_raw_json_to_account(
            &request.body,
...
pub(super) struct RawJsonEndpointRequest {
    ...
    pub(super) session_affinity: Option<CodexSessionAffinity>,
...
pub(super) struct OpenAiSessionState {
    ...
    #[serde(default)]
    pub(super) client_api_key_id: Option<String>,
    #[serde(default)]
    pub(super) identity_seed: Option<crate::transport::qx_application::CodexIdentitySeed>,
}
...
fn encode_openai_session_capture(
    ...
    encode_openai_session_state(OpenAiSessionState {
        ...
        continuation_scope,
        client_api_key_id: capture.client_api_key_id.clone(),
        identity_seed: capture.identity_seed.clone(),
    })
}
...
pub(super) fn cold_json_response_stream(request: ColdJsonResponse) -> EventStream {
    Box::pin(async_stream::try_stream! {
        ...
        let failure_context = OpenAiFailureContext {
            ...
            is_diagnostic: request.lease.is_diagnostic(),
        ...
        let compact_metering = if request.endpoint_path
            == crate::transport::endpoints::CODEX_RESPONSES_COMPACT_PATH
        {
            Some(super::compact::compact_response_metering(
                &request.body,
                &response.body,
                request.upstream_model.as_ref(),
            )?)
        } else {
            None
        };
        ...
        let response_meta = match request.upstream_model.as_ref() {
            Some(model) => ResponseMeta::new(request.context.request_id().as_str(), model.as_str()),
            None => ResponseMeta::for_provider_endpoint(request.context.request_id().as_str()),
        };
        yield ProviderEvent::canonical(GatewayEvent::Started(response_meta.clone()));
        if let Some((usage, cost)) = compact_metering {
...
pub(super) fn cold_response_stream(response: ColdResponse) -> EventStream {
    ...
    Box::pin(async_stream::try_stream! {
        ...
        let failure_context = OpenAiFailureContext {
            ...
            is_diagnostic: lease.is_diagnostic(),
            allows_account_state_mutation,
        };
