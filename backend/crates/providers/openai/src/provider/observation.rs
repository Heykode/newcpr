...
impl OpenAiResponseObservationState {
    ...
    pub(super) fn observation(
        ...
        // 本地用量、费用和额度预测采用最终发送档位；响应回显只作为独立诊断事实保留。
...
pub(super) fn compile_model_capabilities(model: &CodexCatalogModel) -> ProviderModelCapabilities {
    ...
    let capabilities = ModelCapabilities::new(
        BTreeSet::from([OperationKind::Generate, OperationKind::Compact]),
        None,
    )
...
pub(super) fn map_selection_error(error: CredentialSelectionError) -> ProviderError {
    match error {
        ...
        CredentialSelectionError::AccountWait(reason) => {
            tracing::debug!(reason = %reason, "OpenAI account wait ended before upstream send");
            provider_error(
            ...
            .with_diagnostic(
                gateway_core::error::ProviderDiagnostic::new(
                    "OpenAI account capacity wait ended before upstream send",
                )
                .with_classification("account_capacity_wait", reason.as_str()),
            )
        }
