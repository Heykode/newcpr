...
impl CodexQuotaSchedulingProjection {
    ...
    fn reserve_periodic_refreshes(
        ...
        // Nonzero expired windows share the exhausted-account retry budget.
        // A reset deadline requests new evidence; it does not restore quota.
        let mut reserved = Vec::new();
        for (account, reset_at) in candidates {
            if !periodic_quota_refresh_due(&state, account.id(), reset_at, now, refreshed_at) {
                continue;
            }
...
impl CodexCredentialQuotaService {
    ...
    fn backend_client(&self) -> CodexBackendClient {
        ...
        let client = match &self.request_tuning {
            Some(request_tuning) => client.with_request_tuning(request_tuning.clone()),
            None => client,
        };
    ...
    pub async fn apply_rate_limit_429(
        ...
        let configured_cooldown =
            Duration::from_secs(self.request_tuning().rate_limit_cooldown_seconds);
        let until = observed_at
            .checked_add(retry_after.unwrap_or(configured_cooldown))
