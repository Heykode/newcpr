...
/// 人工提交的新 token set 的一致性投影，不验签，也不消费旧 token 的 fallback。
pub(crate) fn candidate_oauth_metadata(
    secret: &CodexOAuthSecret,
) -> Result<CodexOAuthMetadata, ()> {
    let id = secret
        .id_token
        .as_ref()
        .map(|token| candidate_token_metadata(secrecy::ExposeSecret::expose_secret(token)))
        .unwrap_or_default();
    let access =
        candidate_token_metadata(secrecy::ExposeSecret::expose_secret(&secret.access_token));
    if conflicting_claims(
        id.chatgpt_user_id.as_deref(),
        access.chatgpt_user_id.as_deref(),
    ) || conflicting_claims(
        id.chatgpt_account_id.as_deref(),
        access.chatgpt_account_id.as_deref(),
    ) {
        return Err(());
    }
    Ok(CodexOAuthMetadata {
...
fn candidate_token_metadata(token: &str) -> CodexOAuthMetadata {
    let Ok(claims) = decode_jwt_payload::<serde_json::Value>(token) else {
        return CodexOAuthMetadata::default();
    };
    let auth = &claims["https://api.openai.com/auth"];
    let user = nonempty_claim(&auth["chatgpt_user_id"]);
    let fallback_user = nonempty_claim(&auth["user_id"]);
    // 逐字段读取，避免坏 email/plan 遮蔽可观测的身份冲突；opaque/缺失字段仍是 unknown。
    CodexOAuthMetadata {
        email: nonempty_claim(&claims["email"])
            .or_else(|| nonempty_claim(&claims["https://api.openai.com/profile"]["email"])),
        chatgpt_plan_type: serde_json::from_value::<ChatgptPlanType>(
            auth["chatgpt_plan_type"].clone(),
        )
        .ok()
        .map(ChatgptPlanType::into_raw_value)
        .filter(|value| !value.trim().is_empty()),
        chatgpt_user_id: user.or(fallback_user),
        chatgpt_account_id: nonempty_claim(&auth["chatgpt_account_id"]),
    }
}

fn nonempty_claim(value: &serde_json::Value) -> Option<String> {
    value
        .as_str()
        .filter(|value| !value.trim().is_empty())
        .map(str::to_owned)
...
pub(crate) fn conflicting_claims(left: Option<&str>, right: Option<&str>) -> bool {
