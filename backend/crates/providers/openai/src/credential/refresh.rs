        ...
        match result {
            Ok(revision) => {
                if let Some(runtime) = &self.egress_runtime
                    && runtime.reload().await.is_err()
                {
                    // The credential CAS already committed; fencing stale egress must not
                    // turn a successful rotation into another token-refresh attempt.
                    tracing::warn!(
                        "OpenAI egress state could not reload after successful credential refresh"
                    );
                }
