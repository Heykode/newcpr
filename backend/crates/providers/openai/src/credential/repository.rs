...
impl CodexCredentialRepository {
    ...
    pub async fn rotate_refreshed_oauth_secret(
        ...
        let current = self
            ...
            .await?;
        // Credential revision is the fence; quota/profile changes can race a successful exchange.
    ...
    /// 诊断固定账号的健康事实写入；允许 disabled，但不改变账号启用状态。
    pub async fn apply_diagnostic_state_with_reason(
        &self,
        account: &ProviderAccount,
        credential_state: CredentialState,
        observed_at: SystemTime,
        ...
        self.store
            .apply_diagnostic_state_change(AccountStateChange {
                account_id: account.id().clone(),
                expected_revision: account.revision(),
                credential_state,
                observed_at,
