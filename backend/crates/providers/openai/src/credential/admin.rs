...
use super::types::{
    ...
    CodexCredentialPrincipal, CodexOAuthMetadata, CodexOAuthSecret, candidate_oauth_metadata,
    conflicting_claims, parse_access_token_expiration,
};
...
pub enum CodexCredentialAdminError {
    ...
    InvalidCredential,
    #[error("Codex credential principal conflicts with the supplied or current identity")]
    PrincipalConflict,
    #[error("Codex credential principal continuity could not be confirmed")]
    PrincipalUnconfirmed,
...
impl CodexCredentialAdmin {
    ...
    /// 受信管理员的原地替换/重授权；先检查新材料，再保留允许缺失的旧 token。
    pub(crate) fn prepare_candidate_oauth_rotation(
        ...
        if secret.id_token.is_none() {
            secret.id_token = oauth.id_token.clone().map(SecretString::from);
        }
        secret.refresh_token = secret.refresh_token.or(fallback_refresh_token);
        ...
        let profile = ProviderAccountUpdate {
            ...
            email: metadata
                .email
                .or_else(|| current.account.email().map(str::to_owned)),
            plan_type: metadata
                .chatgpt_plan_type
                .or_else(|| current.account.plan_type().map(str::to_owned)),
        };
    ...
    pub async fn prepare_import_document_with_proxy(
        ...
        for candidate in candidates {
                ...
                .starts_with("at-")
            {
                ...
                candidate_oauth_metadata(&secret)
                    .map_err(|()| CodexCredentialAdminError::PrincipalConflict)?
            };
            let prepared =
