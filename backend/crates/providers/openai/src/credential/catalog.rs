...
pub enum CodexCredentialCatalogError {
    ...
    Upstream {
        detail: String,
        status: Option<u16>,
        egress: Option<CodexEgressError>,
    },
...
struct CachedClientCatalog {
    // Start negative-cache backoff after completion, including slow failures.
...
impl CodexCredentialCatalogService {
    ...
    fn backend_client(&self) -> CodexBackendClient {
        self.backend_client_with_profile(self.profile.snapshot())
    }

    fn backend_client_with_profile(&self, profile: CodexWireProfile) -> CodexBackendClient {
        let client = CodexBackendClient::new(
            self.http.clone(),
            self.base_url.clone(),
            CodexWireProfileState::new(profile),
        );
        match &self.egress_runtime {
            Some(runtime) => client.with_egress_runtime(Arc::clone(runtime)),
            None => client,
        }
    }

    /// Select a stable, eligible account within the frozen client scope.
    /// Native objects never use the plan cache or metered execution path.
    pub async fn client_model_catalog(
        ...
        for account in accounts.iter().take(MAX_CATALOG_FETCH_ATTEMPTS) {
            ...
            let egress_revision = self
                .egress_runtime
                .as_ref()
                .map(|runtime| {
                    runtime.check_account(account.id())?;
                    runtime.snapshot().map(|state| state.revision)
                })
                .transpose();
            let egress_revision = match egress_revision {
                Ok(revision) => revision,
                Err(error) => {
                    last_error = catalog_client_error(error.into());
                    continue;
                }
            };
            ...
            let value = {
                ...
                cache.retain(|entry| {
                    Arc::strong_count(&entry.value) > 1 || (!entry.invalidated && entry.is_fresh())
                });
                if let Some(entry) = cache
                    .iter()
                    .find(|entry| !entry.invalidated && entry.is_fresh() && entry.key == key)
                {
                    Arc::clone(&entry.value)
                } else {
                    // Keep in-flight entries to bound concurrent upstream requests.
                    if cache.len() >= MAX_CLIENT_CATALOGS {
                        let Some(index) = cache
                            .iter()
                            .position(|entry| Arc::strong_count(&entry.value) == 1)
                        else {
            ...
            let result = value
                .get_or_init(|| async {
                    // Use exactly the keyed effective profile, preserving account egress.
                    let client = self.backend_client_with_profile(profile);
    ...
    fn invalidate_client_catalogs(&self) -> Result<(), CodexCredentialCatalogError> {
        self.client_catalogs
            ...
            .map_err(|_| CodexCredentialCatalogError::Cache)?
            .retain_mut(|entry| {
                // Fence reuse without forgetting live ownership or exceeding the outbound bound.
...
fn catalog_client_error(error: crate::transport::CodexClientError) -> CodexCredentialCatalogError {
    CodexCredentialCatalogError::Upstream {
        status: catalog_upstream_status(&error),
        detail: error.to_string(),
        egress: match error {
            crate::transport::CodexClientError::Egress(egress) => Some(egress),
            _ => None,
        },
    }
}

fn catalog_upstream_status(error: &crate::transport::CodexClientError) -> Option<u16> {
    match error {
        crate::transport::CodexClientError::Upstream { status, .. } => Some(status.as_u16()),
        _ => None,
    }
}
