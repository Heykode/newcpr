...
use super::egress::{CodexEgressError, CodexEgressRoute, CodexEgressRuntime};
use super::response_meta::CodexResponseMetadata;
use super::tls::{
    CustomCaError, build_reqwest_native_client_with_custom_ca, custom_ca_env_cache_key,
};
use super::websocket::{
...
type ProbeProxyClientCache = Mutex<Option<(ReqwestClientCacheKey, Client)>>;
...
pub enum CodexClientError {
    ...
    /// 本地出口配置错误；不得被当作上游账号额度或认证错误。
    #[error("Codex egress configuration error: {0}")]
    Egress(#[from] CodexEgressError),
    /// 请求头名字无效。
...
impl fmt::Debug for CodexClientError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Egress(_) => formatter.write_str("CodexClientError::Egress([REDACTED])"),
...
pub struct CodexBackendClient {
    ...
    pub(super) egress_runtime: Option<Arc<CodexEgressRuntime>>,
...
impl CodexBackendClient {
    ...
    pub fn with_request_tuning(mut self, request_tuning: RequestTuningHandle) -> Self {
        self.websocket_origin_breaker = self
            .websocket_origin_breaker
            .clone()
    ...
    pub(super) fn request_tuning(&self) -> gateway_core::routing::RequestTuning {
        self.request_tuning.as_ref().map_or_else(
            gateway_core::routing::RequestTuning::default,
            RequestTuningHandle::load,
        )
    }
    ...
    fn request_client(
        ...
        if let Some(runtime) = &self.egress_runtime {
            ...
            if let Some(route) = selected {
                return Ok(runtime.http_client(&route, profile)?);
            }
        }
        Ok(self.client.clone())
    }

    pub(super) async fn send_profiled<F>(
        &self,
        profile: &super::profile::CodexWireProfile,
        json: bool,
        build: F,
    ...
    pub(super) fn http_send_error(&self, error: reqwest::Error, json: bool) -> CodexClientError {
        let explicit_egress = self.egress_route.is_some()
            || self
                .egress_runtime
                .as_ref()
                .zip(self.egress_account.as_ref())
                .is_some_and(|(runtime, account)| runtime.active(account));
        if explicit_egress && error.is_connect() {
            CodexClientError::Egress(CodexEgressError::ConnectFailed)
        } else if json {
            CodexClientError::HttpJson(error)
        } else {
...
pub(crate) struct PreparedResponseTransport {
    ...
    pub(super) attempt_client: Option<Box<CodexBackendClient>>,
}
...
pub(super) fn websocket_upstream_request(request: &CodexResponsesRequest) -> CodexResponsesRequest {
    ...
    super::request::normalize_generate_upstream_body(request.body_mut());
...
pub(super) fn websocket_exchange_error_to_client_error(
    ...
    match error {
        CodexWebSocketExchangeError::Egress(error) => CodexClientError::Egress(error),
