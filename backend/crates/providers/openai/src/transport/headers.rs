...
use super::request::generate_upstream_service_tier;
...
impl CodexBackendClient {
    ...
    pub(super) fn account_request_headers(
        ...
        let mut headers =
            build_codex_account_headers(profile, context.authorization, context.account_id)?;
        insert_optional_header(&mut headers, "cookie", context.cookie_header)?;
        Ok(headers)
    }
    ...
    fn response_headers(
        ...
        let routing_hint = match request.service_tier().map(generate_upstream_service_tier) {
