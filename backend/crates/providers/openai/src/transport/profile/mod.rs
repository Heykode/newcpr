...
impl CodexWireProfile {
    /// 自动识别受支持的完整 UA；识别本身不选择 TLS 或应用层策略。
    pub fn parse_custom_user_agent(
        value: &str,
        desktop: &Self,
    ) -> Result<Self, CodexWireProfileError> {
        let mut profile = if value.starts_with("Codex Desktop/") {
            Self::parse_user_agent(value)?
        } else {
    ...
    /// 将 Core 请求使用的复合 UA 解析为一个完整、配套的 Desktop 画像。
    ///
    /// 这里故意只接受 CPR 自己支持的 Codex Desktop 语法；任意浏览器 UA
    /// 不能被当作 Codex 画像使用，也不会因此改变 TLS 或设备身份。
    /// 系统/架构检查仅限定受支持的文本组合，不证明 TLS 与该平台实测一致。
    pub fn parse_user_agent(value: &str) -> Result<Self, CodexWireProfileError> {
        if value.len() > 512
            || !value.is_ascii()
        ...
        let (originator, codex_version) = product
            .rsplit_once('/')
            .ok_or(CodexWireProfileError::InvalidSyntax)?;
        let (target, tail) = tail
            .split_once(") ")
            .ok_or(CodexWireProfileError::InvalidSyntax)?;
        let (terminal, client) = tail
            .split_once(" (")
            .ok_or(CodexWireProfileError::InvalidSyntax)?;
        let client = client
            .strip_suffix(')')
            .ok_or(CodexWireProfileError::InvalidSyntax)?;
        let (client_originator, desktop_version) = client
            .split_once("; ")
            .ok_or(CodexWireProfileError::InvalidSyntax)?;
        if originator != client_originator || originator != "Codex Desktop" || !safe_token(terminal)
        {
            return Err(CodexWireProfileError::Incoherent);
        }
        let (os_type, target) = ["Mac OS", "Windows", "Linux"]
            .iter()
            .find_map(|os| {
                target
        ...
        let (os_version, arch) = target
            .split_once("; ")
            .ok_or(CodexWireProfileError::InvalidSyntax)?;
        if !matches!(os_type, "Mac OS" | "Windows" | "Linux")
            || !safe_token(os_version)
            || !os_version
                .as_bytes()
                .first()
                .is_some_and(u8::is_ascii_digit)
            || !safe_token(arch)
            || !valid_architecture(os_type, arch)
            || semver::Version::parse(codex_version).is_err()
            || !numeric_dotted_version(desktop_version)
        {
            return Err(CodexWireProfileError::Incoherent);
        }
        Ok(Self {
    ...
    pub fn user_agent(&self) -> String {
        if let Some(raw) = &self.raw_user_agent {
            return raw.clone();
        }
        format!(
            "{}/{} ({} {}; {}) {} ({}; {})",
    ...
    pub fn desktop_user_agent(&self) -> String {
        let originator = if self.raw_user_agent.is_some() {
            "Codex Desktop"
        } else {
        ...
        format!(
            "{originator}/{} ({}; {})",
            self.desktop_version, self.os_type, self.arch,
        )
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, thiserror::Error)]
pub enum CodexWireProfileError {
    #[error("UA syntax is invalid")]
    InvalidSyntax,
    #[error("UA contains unsafe characters")]
    Unsafe,
    #[error("UA fields are incoherent")]
    Incoherent,
}
...
fn valid_architecture(os_type: &str, arch: &str) -> bool {
    match os_type {
        "Mac OS" => matches!(arch, "arm64" | "x86_64"),
        "Windows" => matches!(arch, "x86_64" | "arm64"),
        "Linux" => matches!(arch, "x86_64" | "aarch64" | "arm64"),
        _ => false,
    }
}
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CodexWireProfileSettings {
    pub mode: CodexWireProfileOverride,
    pub default_user_agent: String,
    pub effective_user_agent: String,
    pub effective_desktop_user_agent: String,
    pub effective_desktop_version: String,
    pub default_verified_at: DateTime<Utc>,
    pub effective_profile: CodexWireProfile,
}
...
impl CodexWireProfileSelection {
...
impl CodexWireProfileState {
    ...
    pub fn new(profile: CodexWireProfile) -> Self {
        Self {
            selection: Arc::new(RwLock::new(CodexWireProfileSelection {
                default_profile: profile,
                mode: CodexWireProfileOverride::Default,
            })),
        }
    }
    ...
    /// 一次读锁冻结有效选择和独立 Desktop 默认，后续发布不会改变这份状态。
    ...
    /// 返回当前已核验的默认画像，不受自定义覆盖影响。
    pub fn default_snapshot(&self) -> CodexWireProfile {
        self.selection
            .read()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .default_profile
    ...
    /// 返回管理页面所需的默认、模式与实际生效 UA 预览。
    pub fn settings_snapshot(&self) -> CodexWireProfileSettings {
        let selection = self
            .selection
            .read()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let default_profile = &selection.default_profile;
        let effective = selection.effective();
        CodexWireProfileSettings {
            mode: selection.mode.clone(),
            default_user_agent: default_profile.user_agent(),
            effective_user_agent: effective.user_agent(),
            effective_desktop_user_agent: selection.desktop().desktop_user_agent(),
            effective_desktop_version: selection.desktop().desktop_version.clone(),
            default_verified_at: default_profile.verified_at,
            effective_profile: effective.clone(),
        }
    }

    /// The stored command is parsed by the same owner on save and on startup.
    pub fn apply_user_agent_override(
        &self,
        selection: &ProviderUserAgentOverride,
    ) -> Result<(), CodexWireProfileError> {
    ...
    pub fn update_bundled_release(&self, release: &CodexBundledReleaseProfile) {
        let mut selection = self
            ...
            .write()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let profile = &mut selection.default_profile;
