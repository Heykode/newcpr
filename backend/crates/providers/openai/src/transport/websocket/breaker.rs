...
impl WebSocketOriginBreaker {
    ...
    fn effective_config(&self) -> WebSocketOriginBreakerConfig {
        self.request_tuning.as_ref().map_or(self.config, |handle| {
            let tuning = handle.load();
            WebSocketOriginBreakerConfig {
                failure_threshold: tuning.websocket_failure_threshold.max(1) as usize,
                failure_window: Duration::from_millis(tuning.websocket_failure_window_ms),
                open_duration: Duration::from_millis(tuning.websocket_failure_open_duration_ms),
            }
        })
    }
    ...
    pub fn try_acquire(&self, origin_key: &str) -> WebSocketOriginBreakerDecision {
        ...
        match state {
            CircuitState::Closed { failures } => {
                retain_window(failures, now, config.failure_window);
    ...
    fn record_failure(&self, origin_key: &str, probe_id: Option<Uuid>) {
        ...
        if failures.len() >= config.failure_threshold {
