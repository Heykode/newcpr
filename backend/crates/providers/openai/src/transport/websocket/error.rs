...
pub enum CodexWebSocketExchangeError {
    ...
    /// Local source configuration failed before any upstream application data.
    #[error("websocket local egress is unavailable: {0}")]
    Egress(crate::transport::egress::CodexEgressError),
    ...
    /// The stream ended without a Close frame; only safe pump facts are retained.
...
impl CodexWebSocketExchangeError {
    ...
    pub(crate) fn is_message_too_big(&self) -> bool {
        self.close_before_terminal().and_then(|close| close.code()) == Some(1009)
    }
