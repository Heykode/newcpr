...
pub(crate) struct OpenAiAdminProvider {
    egress_runtime: Option<Arc<crate::transport::egress::CodexEgressRuntime>>,
...
impl OpenAiAdminProvider {
    ...
    #[must_use]
    pub(crate) fn with_egress_runtime(
        mut self,
    ...
    async fn reload_egress(&self) -> Result<(), ProviderAdminError> {
        if let Some(runtime) = &self.egress_runtime {
            runtime.reload().await.map_err(|_| {
                ProviderAdminError::new(ProviderAdminErrorKind::Unavailable)
                    .with_public_message("出口配置已保存，但运行状态刷新失败，请检查服务状态")
            })?;
            self.websocket_pool
                .retire_unavailable_egress(runtime)
                .await
                .map_err(|_| {
                    ProviderAdminError::new(ProviderAdminErrorKind::Unavailable)
                        .with_public_message("出口配置已保存，但连接状态刷新失败，请检查服务状态")
                })?;
        }
        Ok(())
    }
...
impl ProviderAdmin for OpenAiAdminProvider {
    ...
    fn preview_outbound_user_agent(
        ...
        crate::admin_user_agent::preview(&self.profile, selection)
    }
    ...
    fn apply_outbound_user_agent(
        ...
        let previous_user_agent = self.profile.snapshot().user_agent();
        let view = crate::admin_user_agent::apply(&self.profile, &selection)?;
        // Periodic reconciliation of an unchanged selection must keep warm pools.
        if view.effective_user_agent != previous_user_agent {
    ...
    async fn egress_configuration_changed(&self) -> Result<(), ProviderAdminError> {
        self.reload_egress().await
    }

    fn validate_egress_sources(
        &self,
        addresses: &[std::net::Ipv6Addr],
    ) -> Result<(), ProviderAdminError> {
        for address in addresses {
            crate::transport::egress::validate_local_source(*address).map_err(|_| {
                ProviderAdminError::new(ProviderAdminErrorKind::Invalid)
                    .with_public_message("所选 IPv6 尚未在当前服务器配置，或当前进程无法绑定")
            })?;
        }
        Ok(())
    }
    ...
    fn connection_test_operation(
        ...
        build_connection_test_operation(upstream_model, input_text, true)
    }

    fn connection_test_operation_with_options(
        &self,
        upstream_model: &UpstreamModelId,
        input_text: &str,
        ...
        if endpoint != ConnectionTestEndpoint::Responses {
            return Err(provider_admin_error(ProviderAdminErrorKind::Unsupported)
                .with_public_message(
                    "当前 OpenAI Provider 仅支持 Responses 连接测试，不支持 Completions",
                ));
        }
    ...
    fn dashboard_wire_profile(&self) -> Option<DashboardWireProfile> {
        ...
        let verified = matches!(
            selection.mode,
            crate::transport::profile::CodexWireProfileOverride::Default
        );
        ...
        let release = dashboard_desktop_release(&self.profile.default_snapshot(), release);
    ...
    async fn relogin(
        ...
        crate::credential::relogin::login(self, request).await
    }
    ...
    async fn prepare_import(
        ...
        for account in prepared.into_accounts() {
            // Durable device selection belongs to the same Store transaction
            // as the principal upsert, not an unfenced lookup by a new row ID.
    ...
    async fn prepare_rotation(
        ...
        let prepared = CodexCredentialAdmin
            .prepare_candidate_oauth_rotation(current, secret, access_token_expires_at, None)
            .map_err(map_credential_admin_error)?;
...
fn map_credential_admin_error(error: CodexCredentialAdminError) -> ProviderAdminError {
    ...
    let (kind, public_message) = match error {
        ...
        Error::PrincipalConflict => (
            Kind::Conflict,
            "新凭据的账号主体与其他新令牌或当前账号不一致，请检查账号后重试",
        ),
        Error::PrincipalUnconfirmed => (
            Kind::Conflict,
            "无法确认新凭据与当前账号的主体连续性，请提供完整账号资料或使用新建授权",
        ),
...
const fn credential_admin_error_code(error: &CodexCredentialAdminError) -> &'static str {
    match error {
        ...
        CodexCredentialAdminError::PrincipalConflict => "principal_conflict",
        CodexCredentialAdminError::PrincipalUnconfirmed => "principal_unconfirmed",
...
fn map_oauth_error(error: CodexOAuthAdminError) -> ProviderAdminError {
    ...
    let (kind, message) = match error {
        ...
        Error::PrincipalConflict => {
            return map_credential_admin_error(CodexCredentialAdminError::PrincipalConflict);
        }
        Error::PrincipalUnconfirmed => {
            return map_credential_admin_error(CodexCredentialAdminError::PrincipalUnconfirmed);
        }
...
const fn oauth_error_code(error: &CodexOAuthAdminError) -> &'static str {
    match error {
        ...
        CodexOAuthAdminError::PrincipalConflict => "principal_conflict",
        CodexOAuthAdminError::PrincipalUnconfirmed => "principal_unconfirmed",
    ...
    let (kind, message) = match error {
        ...
        Error::Upstream { status, .. } => {
            let message = match status {
                Some(401 | 403) => {
                    "OpenAI 拒绝了用于获取模型列表的账号凭据。请先刷新 Token；仍失败再重新授权。"
                }
                Some(429) => "OpenAI 暂时限制了模型列表请求，请稍后重试。",
                Some(500..=599) => "OpenAI 模型列表服务暂时不可用，请稍后重试。",
                Some(_) => "OpenAI 返回了模型列表错误，请检查账号配置后重试。",
                None => "无法连接 OpenAI 模型列表服务，请检查服务器网络或代理后重试。",
            };
            return provider_admin_error(ProviderAdminErrorKind::BadGateway)
