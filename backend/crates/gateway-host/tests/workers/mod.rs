...
use gateway_core::task::{
    ...
    WorkerDisabledReason, WorkerFencingToken, WorkerId, WorkerKind, WorkerLeaderLeaseGuard,
...
async fn shutdown_deadline_joins_cancelled_daemon_and_drops_held_guard() {
    ...
    let worker = WorkerId::try_new(WorkerKind::OpsFlush, "held_cleanup").unwrap();
    let mut plan = complete_plan(Vec::new());
    plan.retain(|contribution| contribution.kind() != WorkerKind::OpsFlush);
    plan.push(WorkerContribution::Registration(
    ...
    let supervisor = WorkerSupervisor::new(cancellation.clone());
    supervisor
        .start(plan, Arc::new(FakeLeasePort::default()))
        .expect("complete plan");
    yield_until(|| task.active.load(Ordering::SeqCst) == 1).await;

    // Host cancellation can precede shutdown while HTTP connections drain.
    cancellation.cancel();
    yield_until(|| task.observed_cancellation.load(Ordering::SeqCst)).await;
    tokio::time::advance(Duration::from_secs(36)).await;
    assert_eq!(task.active.load(Ordering::SeqCst), 1);

    let started = tokio::time::Instant::now();
    let shutdown = supervisor.shutdown(Duration::from_millis(100));
    tokio::pin!(shutdown);
    ...
    tokio::time::timeout(Duration::from_secs(1), shutdown)
        .await
        .expect("supervisor must abort and join the hung daemon");
    assert_eq!(started.elapsed(), Duration::from_millis(100));
    assert_eq!(
        task.active.load(Ordering::SeqCst),
        0,
        "shutdown must join the aborted daemon, not leave its guard alive"
    );
    assert_eq!(
        task_health(&supervisor, &worker).state,
        WorkerRuntimeState::Stopped
    );
}
