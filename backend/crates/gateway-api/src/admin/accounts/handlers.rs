    ...
    S: AdminSessionState + Clone + Send + Sync + 'static,
{
    Router::new()
        ...
        .route(
            ...
            get(test_account_connection::<S>).post(post_account_connection_test::<S>),
        )
        .route(
...
where
    S: AdminSessionState + Send + Sync,
{
    account_connection_test_response(state, query).await
}

async fn post_account_connection_test<S>(
    _auth: AdminAuth,
    State(state): State<S>,
    AdminJson(query): AdminJson<AccountTestQuery>,
) -> Result<Sse<impl Stream<Item = Result<Event, Infallible>>>, AdminError>
where
    S: AdminSessionState + Send + Sync,
{
...
async fn account_connection_test_response<S>(
    state: S,
