...
impl BatchUpdateAccountsRequest {
    pub fn validate(&self) -> Result<(), WireValidationError> {
        ...
        if let Some(concurrency_limit) = self.concurrency_limit {
            parse_concurrency_limit(concurrency_limit)?;
        }
        if let Some(weight) = self.weight {
            parse_account_weight(weight)?;
        }
        proxy_selection(
        ...
        {
            return Err(WireValidationError::new("fields"));
        }
        Ok(())
    }
...
impl ListQuery {
    ...
    pub fn validate(self) -> Result<AccountListQuery, WireValidationError> {
        ...
        let plan_type = self
            ...
            .map(|value| value.trim().to_owned())
            .filter(|value| !value.is_empty());
        if plan_type.as_deref().is_some_and(|value| {
            value.len() > MAX_SEARCH_BYTES || value.chars().any(char::is_control)
        }) {
            return Err(WireValidationError::new("planType"));
        }
...
fn parse_sort_field(value: &str) -> Option<AccountSortField> {
    match value.trim() {
        ...
        "reloginCount" => Some(AccountSortField::ReloginCount),
        "addedAt" | "createdAt" => Some(AccountSortField::CreatedAt),
...
pub struct AccountTurnStateModelStatusView {
    ...
    pub probe_total_attempts: u64,
    pub probe_cooldown_until: Option<String>,
    pub probe_retry_from_upstream: Option<bool>,
    pub probe_http_status: Option<u16>,
    pub probe_error_code: Option<String>,
    pub probe_returned_length: Option<u16>,
...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct AccountHealthBucketView {
    pub key: String,
    pub start_at: String,
...
pub struct AccountUsageView {
    pub window_label_display: String,
    pub quota_window: Option<AccountUsageQuotaWindowView>,
...
/// 与账号本轮消费配对的额度窗口，不是历史预测。
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct AccountUsageQuotaWindowView {
    pub key: String,
    pub period: &'static str,
    pub used_percent: Option<f64>,
    pub reset_at: Option<DateTime<Utc>>,
...
pub struct AccountTestQuery {
    pub account_id: String,
    pub model_id: String,
    #[serde(default = "default_connection_test_interface")]
    pub interface: String,
    #[serde(default = "default_connection_test_stream")]
    pub stream: bool,
    #[serde(default = "default_connection_test_prompt")]
    pub prompt: String,
}
...
impl AccountTestQuery {
    pub fn validate(&self) -> Result<(), WireValidationError> {
        ...
        if !matches!(self.interface.as_str(), "responses" | "completions") {
            return Err(WireValidationError::new("interface"));
        }
        if self.prompt.trim().is_empty()
            || self.prompt.len() > MAX_CONNECTION_TEST_PROMPT_BYTES
            || self
                .prompt
                .chars()
    ...
    pub(super) fn into_command(
        self,
    ) -> Result<gateway_admin::model::accounts::AccountConnectionTest, WireValidationError> {
        self.validate()?;
        let endpoint = match self.interface.as_str() {
            "responses" => gateway_admin::model::accounts::ConnectionTestEndpoint::Responses,
            "completions" => gateway_admin::model::accounts::ConnectionTestEndpoint::Completions,
            _ => return Err(WireValidationError::new("interface")),
        };
        Ok(gateway_admin::model::accounts::AccountConnectionTest {
            account_id: ProviderAccountId::new(self.account_id)
                .map_err(|_| WireValidationError::new("accountId"))?,
...
fn default_connection_test_interface() -> String {
    "responses".to_owned()
}

fn default_connection_test_stream() -> bool {
    true
}

fn default_connection_test_prompt() -> String {
...
impl From<DomainConnectionTestEvent> for AccountConnectionTestEvent {
    fn from(event: DomainConnectionTestEvent) -> Self {
        let data = match event {
            DomainConnectionTestEvent::Started { model, endpoint } => serde_json::json!({
                "type": "test_start",
                "model": model,
                "interface": endpoint.as_str(),
                "text": format!("正在连接上游 {}", endpoint.label())
            }),
