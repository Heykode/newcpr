    ...
    S: AdminSessionState + Clone + Send + Sync + 'static,
{
    Router::new()
        ...
        .route("/api/admin/request-captures/{id}/stop", post(stop::<S>))
        .route(
        ...
        .route("/api/admin/request-captures/records/{id}", get(read::<S>))
        .route(
    ...
    S: AdminSessionState + Send + Sync,
{
    let page = state
        .admin_services()
        .request_capture()
        .read(&id, query.offset, &auth.context().mutation_context())
        .await
        .map_err(map_admin_service_error)?;
