...
use super::{
    error::{engine_error_response, gateway_error_from_engine, protocol_error_response},
...
async fn collect_raw_json_session(session: Box<dyn ExecutionSession>) -> Response {
    ...
    let body = match raw_json_body(events) {
        ...
        None => {
            ...
            execution.fail_delivery(invalid_upstream_error()).await;
            return response;
        }
    };
    let status = session
...
fn invalid_upstream_error() -> GatewayError {
    GatewayError::new(
        GatewayErrorKind::UpstreamUnavailable,
        "The gateway could not forward the upstream JSON response.",
    )
    .with_client_code("invalid_upstream_response")
}
