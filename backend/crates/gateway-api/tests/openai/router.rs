...
async fn browser_origin_controls_http_admin_sessions_without_configuration() {
    ...
    for (origins, secure) in [
        ...
        (vec!["http://admin.example.test/"], true),
        (vec!["http://admin.example.test/path"], true),
        (vec!["http://admin.example.test?https"], true),
        (vec!["http://admin.example.test#http"], true),
        (vec!["http://user@admin.example.test"], true),
        (vec!["http://admin.example.test:80"], true),
        (vec!["HTTP://admin.example.test"], true),
        (vec![" http://admin.example.test"], true),
        (vec!["file:///admin.example.test"], true),
        (
            vec!["https://admin.example.test,http://admin.example.test"],
            true,
        ),
        (
        ...
        (
            vec!["http://admin.example.test", "http://admin.example.test"],
            true,
        ),
    ] {
        ...
        assert_eq!(attrs.contains(&"Secure"), secure, "{origins:?}");
        for attribute in ["Path=/", "HttpOnly", "SameSite=Lax"] {
            assert!(attrs.contains(&attribute), "{origins:?} {attribute}");
        }
        ...
        assert_eq!(body["data"]["authenticated"], true, "{origins:?}");
        ...
        let body: Value =
            serde_json::from_slice(&to_bytes(response.into_body(), 4096).await.unwrap()).unwrap();
        assert_eq!(body["data"]["authenticated"], false, "{origins:?}");
    }
}
