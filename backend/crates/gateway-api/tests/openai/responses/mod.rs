...
#[test]
fn decoder_should_keep_delivery_mode_independent_of_upstream_transport_selection() {
    for stream in [false, true] {
        for use_websocket in [None, Some(false), Some(true)] {
            let mut body = json!({
                "model": "smart-code",
                "input": "hello",
                "stream": stream,
            ...
            if let Some(use_websocket) = use_websocket {
                body["use_websocket"] = json!(use_websocket);
            }
            let decoded = generate_request(body);

            assert_eq!(decoded.metadata().stream(), stream);
            assert_eq!(openai_wire_body(&decoded)["stream"], json!(stream));
            assert_eq!(
            ...
            assert_eq!(
                openai_protocol_context(&decoded).get("use_websocket"),
                use_websocket.map(Value::Bool).as_ref(),
                "stream={stream}, use_websocket={use_websocket:?}"
            );
        }
    }
}
...
fn downstream_environment_filter_keeps_session_semantics_and_business_bytes() {
    for canonical in [None, Some("canonical-session")] {
        ...
        headers.append(
            "x-future-business",
            HeaderValue::from_bytes(b"\x80\xff").expect("opaque value"),
        );
        let original = headers.clone();
        let body = json!({
            "model": "smart-code",
            "input": "unchanged business content",
        ...
        for decoded in [
            ...
            for name in [
                ...
                assert!(entries.iter().all(|entry| entry[0] != name), "{name}");
            }
            for name in [
                "thread-id",
                "x-client-request-id",
                ...
                assert!(entries.contains(&json!([name, STANDARD.encode(b"keep")])));
            }
        ...
        assert_eq!(
            headers, original,
            "CORS and local observation retain the input"
        );
    }
}
...
#[test]
fn buffered_recovery_must_not_repair_native_sse_or_websocket_terminal_frames() {
    let events = [
        openai_wire_event(
            vec![],
    ...
    for event in &events {
        let expected = event.wire_event().unwrap().data();
        let messages = websocket.push_websocket(event);
        assert_eq!(messages, [expected.to_string()]);
    ...
    assert_eq!(sse.finish().unwrap()["output"], json!([]));
    assert_eq!(websocket.finish().unwrap()["output"], json!([]));
}
