...
#[tokio::test]
async fn proxy_probe_returns_result_without_creating_a_saved_proxy() {
    let fixture = AdminTestFixture::new().await;
    fixture.auth.insert_session("valid-session");
    let (status, tested) = request(
        &fixture,
...
#[tokio::test]
async fn proxy_probe_does_not_modify_existing_connection_or_test_results() {
    let fixture = AdminTestFixture::new().await;
    fixture.auth.insert_session("valid-session");
    let (status, created) = request(
        &fixture,
        "/api/admin/proxies/create",
        Some(json!({
            "name": "Existing proxy", "proxyUrl": "http://saved.example:8080"
        })),
        true,
    )
    .await;
    ...
    assert_eq!(status, StatusCode::OK);
    let (_, listed) = request(&fixture, "/api/admin/proxies", None, true).await;
    assert_eq!(listed["data"]["items"], json!([created["data"]["record"]]));
}

#[tokio::test]
async fn proxy_probe_rejects_empty_invalid_or_ambiguous_input() {
    let fixture = AdminTestFixture::new().await;
    fixture.auth.insert_session("valid-session");
    for (body, expected) in [
        ...
        (
            json!({"proxyUrl":"http://proxy.example", "id":"proxy_test"}),
            StatusCode::UNPROCESSABLE_ENTITY,
        ),
        (
            json!({"proxyUrl":"http://proxy.example", "targetUrl":"https://other.example"}),
            StatusCode::UNPROCESSABLE_ENTITY,
        ),
    ] {
        assert_eq!(
