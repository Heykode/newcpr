...
impl DefaultOpenAiService {
    ...
    async fn import(
        ...
        let command = CredentialImportCommit {
            outbound_proxy,
            prepared,
            settings: command.settings,
        };
        let result = if require_new {
            self.accounts
                .commit_new_credential_import(command, &context)
                .await
        } else {
            self.accounts
                .commit_credential_import(command, &context)
                .await
        }
