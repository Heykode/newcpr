...
use gateway_admin::{
    ...
    model::{
        ...
        accounts::{
            AccountCumulativeCost, AccountListQuery, AccountPage, AccountRuntimeSnapshot,
...
impl AdminHarness {
    ...
    pub(super) fn relogin(
        mut self,
        store: Arc<dyn gateway_admin::ports::relogin::ReloginStore>,
    ) -> Self {
    ...
    pub(super) async fn build_bundle(self) -> gateway_admin::AdminBundle {
        let mut stores = AdminStorePorts::new(
...
impl AccountGroupStore for UnavailableAccountGroupStore {
    ...
    async fn save_group_monitor(
        &self,
        _: &gateway_admin::model::group_monitor::GroupMonitorReport,
        _: u64,
    ) -> AdminStoreResult<()> {
        Err(unavailable("group monitor snapshot"))
    }
...
impl AccountStore for UnavailableStore {
    ...
    async fn load_account_cumulative_costs(
        ...
        Err(unavailable("account cumulative costs"))
    }
