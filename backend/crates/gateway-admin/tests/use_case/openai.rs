...
#[tokio::test]
async fn principal_rejection_stops_rotation_before_commit_or_success_audit() {
    for message in [
        ...
        let store = FakeAccountStore::new("openai", events.clone());
        let services = service(provider.clone(), store.clone()).await;
        provider.fail_next_with_public_message(ProviderAdminErrorKind::Conflict, message);
        let error = services
            .openai()
            .rotate(RotateCredential {
                mutation: gateway_admin::model::provider_credentials::CredentialMutation {
                    context: context("principal-rotation-rejected"),
                    account_id: ProviderAccountId::new("acct_test").unwrap(),
                },
                provider_material: document(),
            })
            .await
            .expect_err("principal rejection must stop the mutation");
        assert_eq!(
            recorded(&events),
            ["store.credential_details", "provider.prepare_rotation"]
        );
        assert!(store.audit_requests().is_empty());
...
#[tokio::test]
async fn principal_rejection_stops_reauthorization_before_commit_or_success_audit() {
    for message in [
        "新凭据的账号主体与其他新令牌或当前账号不一致，请检查账号后重试",
        "无法确认新凭据与当前账号的主体连续性，请提供完整账号资料或使用新建授权",
    ] {
        ...
        services
            ...
            .start_authorization(StartAuthorization {
                outbound_proxy: None,
                context: context("principal-reauthorization"),
                name: "reauthorize".to_owned(),
                reauthorization: Some(ProviderAccountId::new("acct_test").unwrap()),
            })
            .await
            .unwrap();
        provider.fail_next_with_public_message(ProviderAdminErrorKind::Conflict, message);
