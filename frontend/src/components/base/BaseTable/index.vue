...
defineExpose({
  getScrollElement: () => scrollbarRef.value?.wrapRef ?? undefined,
  getTableElement: () => tableRef.value ?? undefined,
})
</script>
    ...
    <div v-loading="loading" class="relative flex min-h-0 max-w-full flex-1 overflow-hidden">
      ...
      >
        <table ref="table" class="table-fixed border-separate border-spacing-0 text-left" :style="resolvedTableStyle">
          <colgroup>
            <col v-for="column in computedColumns" :key="column.key" :style="columnStyle(column, computedColumns, columnLayout)">
          </colgroup>
          <thead>
            <tr :class="headerRowClass">
              ...
              >
                <div :class="cellContentClass(column)">
                  ...
                  >
                    <span data-column-label class="truncate">
                      <slot :name="`header-${column.key}`" :column="column">{{ column.label }}</slot>
                    </span>
                    <span data-sort-indicator class="inline-flex shrink-0 -translate-y-px flex-col gap-px">
                      <Triangle
