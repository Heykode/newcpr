...
import { quotaWindowCode, resolveAccountUsageWindowPresentation } from '../AccountUsageWindow/presenter'
...
const hasQuotaWindow = computed(() => detailItems.value.some(item => !item.local))
</script>
...
<template>
  <BasePopover class="w-full" trigger="hover-click" placement="right" :hover-delay="240">
    ...
    <section class="w-84 max-w-[calc(100vw-2rem)] overflow-hidden rounded-cp-lg" role="dialog" :aria-label="`${detailTitle}详情`">
      ...
      <div class="grid gap-4" :class="detailHeading ? 'px-3 pt-3 pb-3' : 'px-4 pt-4 pb-3'">
        <article v-for="item in detailItems" :key="item.key">
          <div class="grid grid-cols-[32px_minmax(0,1fr)_auto] items-center gap-2.5">
            <span
              class="grid h-5 w-8 place-items-center rounded-sm bg-cp-blue-container-strong font-mono text-[9px] leading-none font-heavy text-cp-blue-on-container"
            >
              {{ item.code }}
            </span>
          ...
          <dl class="mt-2.5 grid grid-cols-[auto_minmax(0,1fr)] gap-x-4 gap-y-1.5 text-[10px] leading-4">
            ...
            <template v-else>
              ...
              <template v-if="item.reset">
                ...
                <dd
                  class="m-0 text-right font-emphasis tabular-nums text-cp-text-secondary"
                  :class="{ 'text-cp-warning-text': item.reset.pending }"
                  :title="item.reset.title"
                >
                  {{ item.reset.display }}{{ item.reset.pending ? '' : '后' }}
                </dd>
              </template>
              <dt class="font-emphasis text-cp-text-tertiary">
                重置时间
