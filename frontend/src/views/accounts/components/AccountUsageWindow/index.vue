...
<template>
  <div :class="view.classes.root">
    <template v-if="view.mode === 'quota' && window">
      ...
      >
        <span
          v-if="view.quota.reset"
          class="mr-2 tabular-nums"
          :class="{ 'text-cp-warning-text': view.quota.reset.pending }"
          :title="view.quota.reset.title"
        >
          {{ view.quota.reset.display }}{{ view.quota.reset.pending ? '' : '后重置' }}
        </span>
        <span class="break-words">重置时间: {{ window.resetAtDisplay }}</span>
      </div>
    </template>
