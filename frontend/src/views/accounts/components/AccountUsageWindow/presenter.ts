...
export function resolveAccountUsageWindowPresentation(
  ...
  const quota = input.window
    ...
    : null
  const reset = quotaWindowResetPresentation(input.window, input.now)
  ...
  return {
    ...
    quota: {
      ...
      resetVisible: Boolean(input.window)
        ...
        && (Boolean(reset) || input.window?.resetAtDisplay !== '—'),
      reset,
    },
    local: {
...
export function quotaWindowCode(windowSeconds: number | null, role: AccountQuotaWindow['role']) {
...
export function quotaWindowResetPresentation(
  window: Pick<AccountQuotaWindow, 'resetAt' | 'resetAtDisplay'> | undefined,
  now: number,
) {
  const resetAt = window?.resetAt
  // Only the timezone-qualified API timestamp is safe for a countdown.
  if (typeof resetAt !== 'string' || !/T.*(?:Z|[+-]\d{2}:\d{2})$/i.test(resetAt))
    return null
  ...
  if (pending)
    ...
    display = `${Math.floor(minutes / (24 * 60))}d ${Math.floor(minutes / 60) % 24}h`
  else if (minutes >= 60)
    display = `${Math.floor(minutes / 60)}h ${minutes % 60}m`
  else
    display = minutes > 0 ? `${minutes}m` : '<1m'

  const absolute = window?.resetAtDisplay && window.resetAtDisplay !== '—'
    ? `${window.resetAtDisplay} (UTC+8)`
    : resetAt
  return {
    display,
    pending,
    title: `重置时间：${absolute}${pending ? '；已到重置时间，等待上游确认额度恢复' : ''}`,
  }
}
...
function requestTimeline(
  ...
  for (const bucket of buckets) {
    ...
    bucketCounts.set(
      bucketHour,
      (bucketCounts.get(bucketHour) ?? 0) + Math.max(0, bucket.requestCount),
    )
  }
