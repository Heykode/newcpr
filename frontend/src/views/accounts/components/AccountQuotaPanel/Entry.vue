      ...
      <div v-for="item in items" :key="item.key" class="grid min-w-0 gap-1.5">
        ...
        <template v-else>
          ...
          <p class="m-0 flex min-w-0 items-center justify-between gap-2 text-[10px] leading-3.5 text-cp-text-tertiary">
            ...
            <span
              class="min-w-0 truncate font-emphasis tabular-nums"
              :class="{ 'text-cp-warning-text': item.reset?.pending }"
              :title="item.reset?.title ?? item.resetAtDisplay"
            >
              {{ item.reset ? `${item.reset.display}${item.reset.pending ? '' : '后'}` : item.resetAtDisplay }}
            </span>
          </p>
        </template>
      </div>
