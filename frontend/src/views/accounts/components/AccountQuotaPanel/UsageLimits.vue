...
import { quotaWindowPresentation, quotaWindowResetPresentation } from '../AccountUsageWindow/presenter'
  ...
  <section class="overflow-hidden rounded-cp bg-cp-fill-quaternary" aria-label="使用限额">
    ...
    >
      <div class="min-w-0">
        ...
        <p class="mt-1 mb-0 text-cp-xs leading-normal font-emphasis break-words text-cp-text-secondary">
          <span
            v-if="item.reset"
          ...
          >
            {{ item.reset.display }}{{ item.reset.pending ? '' : '后重置' }}
          </span>
          <span class="font-mono tabular-nums">重置时间：{{ item.resetAtDisplay }}</span>
        </p>
      </div>
