...
import type { AccountForecastCache } from '../../composables/useAccountForecastCache'
...
const props = defineProps<{ account: AccountRow, cache?: AccountForecastCache }>()
  ...
  >
    ...
    <div class="grid min-w-0 grid-cols-[minmax(0,1fr)] gap-4">
      <div class="flex min-w-0 flex-wrap items-center justify-between gap-4">
        <AccountIdentityCell :account="account" title-mode="email" meta-position="secondary" class="w-full sm:w-auto">
          <template #meta>
            <span class="min-w-0 text-cp-xs wrap-anywhere text-cp-text-secondary">{{ account.planTypeDisplay }}</span>
          </template>
        </AccountIdentityCell>
        <BaseSegmented v-model="period" label="预测周期" :options="options" class="w-48" />
      </div>

      <div class="min-w-0" aria-live="polite" :aria-busy="loading || refreshing">
