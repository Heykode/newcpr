    ...
    <div class="grid gap-2">
      ...
      <div class="grid gap-2">
        <div v-for="metric in metrics" :key="metric.label" class="rounded-cp bg-cp-bg-container px-3 py-3">
          ...
          <div class="mt-2 grid grid-cols-[minmax(0,1fr)_20px_minmax(0,1.2fr)] items-center gap-2">
            <span class="min-w-0 font-mono text-cp-sm font-emphasis wrap-anywhere tabular-nums text-cp-text [html[data-theme=light]_&]:font-medium">{{ metric.observed }}</span>
            <ArrowRight class="size-3.5 text-cp-text-tertiary" />
            <strong class="min-w-0 text-right font-mono text-xl font-heavy wrap-anywhere tabular-nums" :class="metric.tone">
