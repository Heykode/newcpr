...
<template>
  <section class="min-w-0 space-y-5" aria-label="导入任务详情">
    ...
    <div>
      <div class="mb-2 flex flex-wrap items-baseline justify-between gap-3 text-xs">
      ...
      <div
        ...
        :aria-valuemin="0"
      >
        <div v-for="status in outcomeOrder" :key="status" :class="itemStates[status].fill" :style="{ width: `${task.total ? task.counts[status] / task.total * 100 : 0}%` }" />
      </div>
    ...
    <div>
      <div v-if="hasAttention" class="mb-3 flex flex-wrap items-center gap-x-4 gap-y-3">
        <p v-if="task.counts.unknown" class="flex min-w-0 items-start gap-2 text-xs leading-relaxed text-cp-warning-text" role="status">
          <CircleAlert class="mt-0.5 size-3.5 shrink-0" />
          待核对条目可能已入库，凭据也可能已轮换。请先核对账号，不要直接重试原 Token。
        </p>
        <BaseSegmented v-model="filter" class="ml-auto" label="筛选导入明细" :options="filterOptions" size="sm" />
      </div>
