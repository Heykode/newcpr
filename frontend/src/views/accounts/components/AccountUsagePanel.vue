...
const cumulativeCosts = computed(() => props.account.cumulativeCosts ?? [])
...
<template>
  <section class="grid gap-4 rounded-lg bg-cp-bg-container p-4 shadow-cp-tertiary xl:min-h-0 xl:grid-cols-[0.52fr_1.48fr]">
      ...
      <div class="mb-3 flex shrink-0 flex-wrap items-center gap-x-4 gap-y-2">
        <div class="flex shrink-0 items-center gap-1">
          ...
          <BaseIconButton
            ...
            @click="emit('forecastRequested', account)"
          >
    ...
    <dl class="m-0 grid min-w-0 grid-cols-[auto_minmax(0,1fr)] items-baseline gap-x-4 border-t border-cp-border-secondary pt-3 xl:col-span-2">
      <dt class="text-cp-sm font-heavy text-cp-text">
        累计消费
      </dt>
      <dd class="m-0 min-w-0 text-right">
        <ul v-if="cumulativeCosts.length > 0" class="m-0 grid min-w-0 list-none gap-1 p-0">
          <li
            v-for="cost in cumulativeCosts"
            :key="cost.currency"
            class="min-w-0 break-all font-mono text-cp-sm font-heavy tabular-nums text-cp-text"
          >
            {{ cost.estimatedAmountDisplay }}
          </li>
        </ul>
        <span v-else class="text-cp-sm font-emphasis text-cp-text-quaternary">
          暂无统计
        </span>
      </dd>
    </dl>
