...
export function useAccountMutations(options: {
  ...
  async function handleBatchDelete() {
    ...
    await batchDeletingAction.run(
      ...
      },
      {
        errorText: false,
        onError: (error) => {
          void loadAccounts({ silent: true }).catch(() => undefined)
          toast.error(
            deletedCount > 0
  ...
  async function handleToggleEnabled(account: AccountRow, enabled: boolean) {
    await togglingAccountIds.run(account.id, async () => {
      try {
        await updateAccount({
