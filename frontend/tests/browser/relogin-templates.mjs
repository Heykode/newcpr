...
async function main() {
  ...
  try {
    ...
    assert.equal(await modal.getByRole('switch', { name: '切换 Turn State 注入', exact: true }).count(), 0)
    ...
    assert.equal(await confirm.locator('dt').filter({ hasText: 'State 开关' }).count(), 0)
