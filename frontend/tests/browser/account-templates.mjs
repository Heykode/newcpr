...
async function main() {
  ...
  try {
    ...
    await dialog.getByRole('switch', { name: '切换 Turn State 注入', exact: true }).locator('..').click()
    ...
    assert.equal(current[0].turnStateInjectionEnabled, true)
    ...
    assert.equal(await dialog.getByRole('switch', { name: '切换 Turn State 注入', exact: true }).isChecked(), true)
    await dialog.getByRole('switch', { name: '切换 Turn State 注入', exact: true }).locator('..').click()
    ...
    await dialog.getByText(name, { exact: true }).waitFor()
    await dialog.getByRole('button', { name: '编辑模板', exact: true }).click()
    assert.equal(await dialog.getByRole('switch', { name: '切换 Turn State 注入', exact: true }).isChecked(), false)
