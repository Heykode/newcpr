...
- Responses HTTP 和 WS opening 不主动添加独立的 `x-codex-installation-id` 请求头，
  下游同名头（包括大小写变体和重复值）也不透传，QX 后处理不重新添加。
  账号设备号仍由现有档案提供，写入正文 `client_metadata` 标准字段；
  已有正文与会话元数据里的设备别名继续同步。设备号仍参与原有会话派生与归属，
  不新建设备，不修改设备绑定锁，也不改变 Models、额度、OAuth 或 raw JSON 入口。
