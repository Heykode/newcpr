...
账号列表返回 `modelAccess: { mode, models }`。单账号编辑、批量编辑及导入的 `settings`
接受同名可选字段：`all` 配合空数组表示不限制；`allowlist` 只允许名单内模型；
`denylist` 禁止名单内模型。后两种模式要求 1–256 个精确模型 ID，每个最多 256 字节，
不接受空 ID、前后空白、控制字符、保留前缀或通配符。匹配全局模型映射后的上游 ID。

省略或 `null` 保留已有政策，新账号默认 `all`。批量只改模型限制时不需提交其他字段。
重新授权、Token 刷新和模板应用不重置政策。导出文件携带政策，文件未携带时保留已有值；
导入表单显式设置的政策优先于文件。更改政策只更新配置版本，不更换凭据、设备或 State。

普通请求在评分和获取账号租约前筛选；粘性、重试及容量等待不能绕过限制。
受管模型仍须满足原有有效 State 门槛。管理员账号诊断与独立 Images/Search
保持既有规则，不扩大本次限制范围；已开始请求使用其冻结配置。
...
逐条结果保留 `accountIds`，并附带以实际入库 ID 为键的 `accountEmails`（邮箱或 `null`）。
邮箱是导入成功时的展示快照，随任务一起过期；读取任务不额外查询账号或上游。
面板优先显示邮箱，无邮箱或旧服务未返回该字段时显示 ID，不依赖账号列表当前分页。
...
OpenAI 本地估算包含 `gpt-6-sol`、`gpt-6-luna` 的标准、Flex、Fast、
缓存读写和长上下文价档，按实际发送的模型计价，不因响应模型名不同改用另一型号的价格。
输入超过 272,000 tokens 时切换长上下文价档；未知模型后缀和未知服务档位不猜测价格。
这不改变上游模型目录、账号模型政策或 State 配置，也不把旧型号别名改指向新型号。
