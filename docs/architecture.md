...
- 面向客户端的 OpenAI Responses、Chat 兼容、Images、standalone Search 和模型目录协议；
- 面向管理员的 `/api/admin/*` 控制面和 Vue 管理端；
- OpenAI 与 xAI 两个编译期 Provider；
- PostgreSQL 持久化、Redis 协调状态以及 S3/R2 数据库备份。

`/v1/chat/completions` 在协议层转换后共用 Responses 执行链路。系统不存在 Provider Instance 层，也不支持通过复制应用容器进行多副本
...
5. 每个 crate 的 `src/` 与 `tests/` 平级，行为测试默认不放在生产源码中。`tests/` 镜像 `src/` 的模块目录形态：例如
...
私有时钟和连接租约的白盒回归有以下精确例外，由架构检查按 crate、文件、名称和
`#[cfg(test)]` 限定；不为测试扩大生产 API，不允许 `#[path]`、`cfg_attr` 或混合生产条件：

- `gateway-core/src/runtime/account_concurrency.rs` 的私有内联 `tests` 模块，
  验证宽限期边界和重复暂停不延长截止时间。
- `providers/openai/src/transport/websocket/pool/mod.rs` 的私有 `tests`、
  `lifecycle_tests` 外置模块，验证并发预留、取消、关闭和资源释放顺序。
- `providers/openai/src/transport/websocket/coordinator.rs` 的
  `shared_opening_deadline_does_not_double_charge_account_capacity_wait` 单测，
  验证等待预算不重复扣除。

以上代码只在测试构建中编译。CI 同时执行库单测和 `tests/main.rs` 集成测试，
不再只运行集成测试；其他位置继续遵守原有模块纪律。

补充会话亲和不属于私有状态例外。其派生、迁移、日志脱敏及持久 `lc_` 身份回归通过公开
Provider 观测/执行接口、Store 端口和隔离初始化测试，位于
`providers/openai/tests/provider/contract/affinity.rs` 及已有合同日志测试，不在生产 `src/` 挂载测试。
...
账号导入、凭据更新或重登提交成功后，通过现有账号事实通知唤醒 State 维护队列；
仍检查开关、账号状态及五账号准入，不另起绕过限制的任务。周期扫描保留为恢复与通知丢失的兜底。
采集按账号的实际套餐分类：Team/Business 及对应 self-serve 别名使用团队形状，个人套餐使用个人形状。

OpenAI Provider 独占采集、封装校验、注入和响应观察，PostgreSQL 原子发布账号与模型的当前值及待切换值。
采集与业务调度分离：最多五个采集账号动态补位，账号内模型并发、同键去重；每个模型批次为
1、5、10、10……，每个账号与模型单批最多 10 个请求，没有总次数或整任务时限。单请求保留超时，真实认证、额度和限流事实仍生效。
持续未成功的账号会持续占用采集席位；这些异步任务共享进程资源，不是独立硬件资源。

探测复用正常请求的编码、账号身份和公共画像，独立按请求轮询 IPv6，不携带旧 State，
不申请业务并发租约，也不修改普通业务出口、粘性评分或线程/cache 编号。
首次只采集当前值，剩余十五分钟采集下一份，剩余一分钟切换；无替代值时停止对应模型的新链，
已发送请求及精确原生续写保留原有归属。WS 内部按账号、模型、版本和截止时间限制新链复用，不把版本写进缓存键。

关闭开关保留缓存及原计时。普通响应 Cookie 材料更新与身份 binding 分离。
同一已确认用户、工作区、设备及套餐的 Token 刷新、重登和重新导入，在凭据事务内保留未过期的模型 State，
不重置采集或过期时间。认证代际仍递增，旧探测、响应观察及错误写入不能影响新凭据。
身份不完整或变化时不继承；已经过期或拒绝清除的 State 不恢复。凭据错误期间保留缓存但暂停使用，
恢复后只对缺失或临近到期的模型安排采集。
新 WS 链按认证握手及 State 版本隔离，不复用旧 Token 的连接；精确续写仍按原连接所有权处理，
线程标识、提示缓存键和调度评分不变。
业务回包只做版本绑定的健康观察，不发布新 State；缺失回包头不代表失效，两次连续异常或明确拒绝触发替换/重采。
封装时间加一小时是本地寿命策略，不是上游保证。列表仅展示安全诊断元数据。
...
后台导入由 Admin 的进程内有界队列持有，Host 注册 `account_import` daemon。
API 负责鉴权、4 MiB/200 项限制和请求指纹，Worker 复用原有 Provider 导入服务；
不在队列层识别账号、生成设备或替代存储事务。任务按管理员主体隔离并轮转执行。
停止只跳过待执行项；关闭时先停收新任务、排空在途项，超出 Host 关闭预算的结果
按不确定处理，不自动重放 RT。队列/幂等结果不持久化，账号提交仍持久化。
...
可选的账号忙时等待由 OpenAI selector 编排，复用既有排序和亲和；Core 为同一请求的所有 attempt
提供共享、单调时钟预算，Store 通过 Redis 管理独立等待登记及执行槽晋升。配置沿既有
`RequestTuning` JSONB 发布，不新建账号队列数据库或粘性状态机。

等待设置和授权范围按请求冻结；候选账号的可用性、凭据、容量与运行信号在等待中复核。
实时容量沿已有 `AccountConcurrencyHandle`，旧请求不能抬高最新账号并发限制。
只读轮询不推进轮转游标、不查询上游额度；等待人数不计入 Smart 的执行负载。
每次只等待一个账号，先获得执行槽再进入 WS 连接池，避免反向持有两种资源。

等待登记具有唯一 token 与原始截止时间；Redis 原子检查人数及晋升，取消清理由 Store 的有界
worker 管理。迟到操作必须受到取消标记约束；基础设施不可用时保守拒绝，不能当成空池或无限容量。
等待不是严格 FIFO，也不承诺 Redis 数据丢失后的多副本强一致。
启用等待的 native 容量退出携带明确恢复限制，不能因普通容量错误自动改绑或跨账号重放。
关闭时保留原选择与恢复行为，已有 WS socket 所有权和发送边界不变。
...
- Admin：分组监控 daemon，每 10 秒复用已有额度/费用事实采样，将最新聚合结果独立持久化；
  普通页面只读共享结果，手动请求与正在运行的采样合并，不修改账号、调度或计费事实；
...
分组监控同样依赖该单副本边界。其采样失败保留旧快照和真实采样时间，下一轮重试，
不把非关键展示的失败升级为网关就绪失败；持久 Plan 学习状态独立于最新监控快照。
...
RUST_MIN_STACK=16777216 cargo +1.97.0 test --manifest-path backend/Cargo.toml --workspace --lib --test main --locked
