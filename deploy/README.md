...
日常源码更新优先使用[已验证镜像部署](verified-deployment.md)：复用 CI 产物、
先准备后切换，并统一处理备份和回滚。不要为了更新单台服务器重复执行完整 Release。
...
当前默认只发布 **Linux AMD64（x86_64）**：包括 Rust 程序
`codex-proxy-rs`、带前端的 Linux AMD64 安装包和 `linux/amd64` Docker 镜像。
适用于 x86_64 Linux 服务器；操作系统名称不是 CPU 架构，
新服务器部署前先运行 `uname -sm` 核对，ARM64 机器不能直接使用该原生程序。

唯一生效的平台清单是 `release/platforms.yaml`。不再默认生成 Linux ARM64
和 macOS ARM64 产物，但不删除它们的源码兼容能力，也不影响本地开发。
前端构建、后端测试、安全检查、镜像验证及签名继续保留。
此调整不改变已经运行的容器、历史发布包、账号数据或运行配置。

以后需要恢复其他平台，在 `platforms` 列表下追加需要的条目：
...
这些是恢复配置示例，不是第二份生效清单。新增前应重新核对 runner、Rust target
与原生依赖，执行 `ruby release/tests/platforms_test.rb` 和工作流检查，再按发布流程
构建、测试新版本；不移动历史标签或覆盖历史发布包。只恢复 Linux ARM64 时，
macOS 构建仍会跳过；恢复 macOS 后其任务自动启用。

发布工作流在展开矩阵前跳过没有目标的非 Docker 构建任务，打包只接受预期跳过；
有目标的任务失败、取消或意外跳过时不得发布。不应通过删除质量门禁来减少耗时。
...
- `CPR_UPDATE_REPOSITORY`：只接受 `owner/repository`；默认 `Heykode/newcpr`。
