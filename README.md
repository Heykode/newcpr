...
[![CI](https://github.com/Heykode/newcpr/actions/workflows/ci.yml/badge.svg)](https://github.com/Heykode/newcpr/actions/workflows/ci.yml)
[![Release](https://img.shields.io/github/v/release/Heykode/newcpr?display_name=tag&sort=semver&style=flat-square)](https://github.com/Heykode/newcpr/releases)
[![GHCR](https://img.shields.io/badge/GHCR-newcpr-2496ED?logo=docker&logoColor=white&style=flat-square)](https://github.com/Heykode/newcpr/pkgs/container/newcpr)
...
> [!NOTE]
> 本项目提供 Responses API 和 `/v1/chat/completions` 兼容入口。Chat 在 CPR 内转换为 Responses，复用原有账号调度和上游传输；完整托管工具语义仍优先使用 Responses，具体边界见[接口文档](docs/api.md#chat-兼容边界)。

## 快速开始

使用 Docker Compose 部署发布镜像 `ghcr.io/heykode/newcpr:latest`，同时启动 PostgreSQL 和 Redis。
默认发布平台为 Linux amd64，需要 Docker Engine、Docker Compose Plugin、curl 和 OpenSSL。已有部署请先看
[升级说明](deploy/README.md#镜像升级与源码构建)，不要覆盖原配置。

> [!IMPORTANT]
> 本仓库以独立源码历史维护，不包含旧仓的 Release 或镜像。首次正式发布前，
> 不要直接执行下面的镜像拉取命令；请按[源码构建说明](deploy/README.md#镜像升级与源码构建)
> 在完整源码目录构建。仓库公开不代表已经发布安装包或部署服务。
...
curl -fsSL https://raw.githubusercontent.com/Heykode/newcpr/main/deploy/compose.yaml \
  -o deploy/compose.yaml
curl -fsSL https://raw.githubusercontent.com/Heykode/newcpr/main/deploy/config.example.yaml \
